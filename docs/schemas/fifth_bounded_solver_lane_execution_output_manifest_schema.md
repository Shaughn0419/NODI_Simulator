# Fifth Bounded Solver Lane Output Manifest Schema

Required role: `fifth_bounded_solver_lane_output_manifest_trace_only`.

The output manifest must describe exactly three rows. It may expose only rank, rank-percentile, pairwise order, and P12 rank delta as final interpretation fields. Solver-native values are trace-only and not final gates.
