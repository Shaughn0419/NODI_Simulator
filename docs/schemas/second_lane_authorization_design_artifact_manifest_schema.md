# P7 Authorization Design Artifact Manifest Schema

Required role: `second_lane_authorization_design_artifact_manifest`.

Every artifact row must carry the P7 guard fields, path status, and hash role. The artifact manifest excludes its own hash to avoid a self-hash cycle.
