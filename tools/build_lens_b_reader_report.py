...
def wavelength_404_660_explanation_table(stats: dict[Route, RouteStats]) -> str:
    geometries = [
        ...
        ((500, 1100), "404 旁路候选宽度；660 在此尺寸是弱参考边界，所以只作机制对照。"),
...
def baseline_normalization_audit_table(stats: dict[Route, RouteStats]) -> str:
    ...
    for width, depth, note in [
        ...
        (500, 1100, "404 旁路宽度；归一化更强，且 660 为弱参考边界。"),
...
def normalization_revision_sensitivity_table(stats: dict[Route, RouteStats]) -> str:
    ...
    for width, depth, note in [
        ...
        (500, 1100, "404 旁路宽度；660 弱参考，必须单独治理。"),
...
- 重要方法修正：Stage B6 使用“每个波长各自用金颗粒基准归一化”的口径。因此它能支持 Tsuyama 锚点诊断和同一波长内的几何排序，但**不能单独支撑 EV 跨波长绝对最终推荐**。跨波长最终推荐必须等待固定 660 基准或实测探测器单位口径的 B7 修正重算。

---

## 0. 先给结论

先把证据等级说清楚：下面的路线排序是 **Stage B6 的 gold-normalized、tau=1 ms、1000 events/case 设计读法**。它可以告诉我们“在当前 Tsuyama 锚定口径里哪一族最稳”，但还不能告诉我们“404 和 660 在绝对 EV 物理检测灵敏度上谁最终胜出”。

- **在当前 gold-normalized 口径里，主推荐不是一个孤立点，而是一族路线：660 nm / 800 nm 宽度，1000-1400 nm 高度。**
- 默认中心可以写成 **660 / 800 x 1100 nm**。
- `660 / 800 x 1000 nm` 是宽粒径 EV 假设下的邻近候选；`660 / 800 x 1400 nm` 是尖锐小 EV 假设下的压力测试候选。
- **404 nm 没有被淘汰，也不能被当前归一化口径简单判负**：它在 tau=1 ms 下确实受益，绝对 Mie 散射也更强；但 Stage B6 的每波长金颗粒归一化会压低这部分优势，所以只能先保留为短波旁路候选，并进入 B7 修正口径重算。
- **488 / 532 不进入最终推荐**：它们只能作为趋势 / 对照。即使 488 在某些表中很高，也不能越过波长治理规则。
- **金颗粒行不参与 EV 推荐**：金颗粒只用来检查 Tsuyama 锚点和参数来源链。

当前证据仍是 **每工况 1000 个事件、单随机种子、低事件数设计证据**。它不是实测校准，不是每工况 10000 个事件的最终验证，也不是跨波长绝对归一化后的最终口径 B 推荐。
...
- 在当前 Stage B6 的 gold-normalized 口径里，660 / 800 宽度族在四个 EV 粒径分布假设下都稳定领先；404 / 500 x 1100 接近但没有超过；488 / 600 x 900 可以高分，但它是对照波长。
- 从方法审查角度看，这还不能结束 404 vs 660 的问题，因为当前归一化会把短波 EV 绝对散射优势除掉一部分。报告后面会给出敏感性表，说明为什么必须做 B7 修正口径。
...
- 必须补做：固定 660 金颗粒基准或实测探测器单位口径的 B7 修正重算；在那之前，404 应保留在最终候选集里。
...
下面这张表把几条关键路线放在同一坐标系里。倍数都以 **660 / 800 x 1100** 为 1.00x。检测率用百分比；通过时间用 ms。请注意：这里的“推荐路线”指当前 gold-normalized Stage B6 口径下的设计排序，不是 B7 修正后的最终跨波长推荐。"""
    )
...
- 这张表不能证明 404 在绝对 EV 跨波长灵敏度上已经输给 660；这个问题需要后面的归一化修正敏感性和 B7 重算来解决。
...
- **结论必须修改**：当前 B6 可以作为 Tsuyama/gold-normalized 诊断和几何排序证据；404 vs 660 的最终波长推荐必须增加 B7 修正口径后再签字。
...
- 因为当前口径会压平短波 EV 散射，404/660 的最终判断必须在 B7 固定 660 基准或实测单位口径下重算。
...
- 404 的最佳旁路更偏向 500 nm 宽度；但即使这样，它仍没有在四个 EV 粒径分布假设中超过 660。
...
> 在 tau=1 ms、单随机种子、每工况 1000 个事件的合成 EV 全网格下，以 Tsuyama 金颗粒锚点归一化的口径 B 支持 660 nm / 800 nm 宽度族和 1000-1400 nm 高度带；404 nm 保留为短波旁路候选；488/532 nm 保留为对照。该结论不是实测校准，不是最终验证，也不是 404/660 绝对 EV 跨波长灵敏度的最终判决。
...
- 当前低事件数、gold-normalized 设计证据支持 660 / 800 x 1000-1400 家族。
- 在当前 gold-normalized B6 表内，404 是可推荐旁路，但没有超过 660。
- 404 的绝对 Mie 散射优势没有被当前 B6 否定；它必须进入 B7 修正口径。
...
| 目标 | 下一步 | 解决的问题 |
| --- | --- | --- |
| 修正跨波长归一化 | 新建 B7 修正口径：固定 660 金颗粒基准的 `global_single_lambda` 重算，或进入实测探测器单位 / 波长响应校准口径 | 把 EV 自身的短波散射优势和 Au 每波长锚点分开，重新判断 404 vs 660 |
| 更强的合成证据 | 对同一 B6 范围重跑每工况 10000 个事件、随机种子 42 | 检查 1000 事件低事件数排序是否稳定 |
| 稳健性 | 若 10000 事件结果改变排序，再补三个随机种子 | 检查随机种子敏感性 |
...
为了让结论能说服读者，B7 不应该只把 B6 多跑几倍事件数。事件数只能降低随机误差，不能修正归一化定义。优先级应该是：
