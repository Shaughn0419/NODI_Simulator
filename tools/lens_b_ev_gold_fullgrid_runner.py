...
NORMALIZATION_LANES = ("per_wavelength_gold", "fixed_660_gold")
...
def run_full(args: argparse.Namespace) -> dict[str, Any]:
    ...
    completed = int(run_state["completed"])
    completed_routes = int(run_state["completed_routes"])
    status = "failed" if run_state["failures"] else "completed"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=["trial", "full"],
        default="trial",
        help="Only trial mode is enabled until the user chooses a seed count.",
    )
    return parser
