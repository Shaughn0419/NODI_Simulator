...
from nodi_simulator.sidewall_simulation_assumption_input_workspace import (  # noqa: E402
    SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_CLAIM_BOUNDARY,
    SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_VERSION,
...
PREFIX = "NODI_PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE"
ARTIFACT_ID = "PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_20260701"
DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_READY_HEADER_ONLY"
)
SIMULATION_ROWS_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_SIMULATION_ROWS_PRESENT_RERUN_REQUIRED"
)
BLOCKED_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_FAIL_CLOSED"
)
SELF_MANIFEST_SHA256 = "SELF_MANIFEST_NOT_SELF_HASHED_BY_DESIGN"
CLAIM_BOUNDARY = SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_CLAIM_BOUNDARY
...
ALLOWED_USE = (
    "create/audit header-only simulation-assumption target CSVs for "
    "the sidewall route decision chain"
)
BLOCKED_USE = "template-as-evidence;unreviewed assumptions as claims;production ingestion"

BUILD_EDIT_PATHS = {
    "nodi_simulator/sidewall_simulation_assumption_input_workspace.py",
    ...
    "tests/test_sidewall_simulation_assumption_input_workspace.py",
    "tests/test_nodi_package_c_sidewall_simulation_assumption_input_workspace.py",
}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create/audit sidewall simulation-assumption input CSV workspace."
    )
    parser.add_argument(
...
def workspace_specs() -> list[SidewallSimulationAssumptionInputWorkspaceSpec]:
    return [
        SidewallSimulationAssumptionInputWorkspaceSpec(
...
def source_files() -> dict[str, Path]:
    return {
        ...
        "workspace_source": PROJECT_ROOT
        / "nodi_simulator/sidewall_simulation_assumption_input_workspace.py",
        "workspace_builder": PROJECT_ROOT
        / "tools/audits/build_nodi_package_c_sidewall_simulation_assumption_input_workspace.py",
        "workspace_tests": PROJECT_ROOT
        / "tests/test_sidewall_simulation_assumption_input_workspace.py",
        "workspace_builder_tests": PROJECT_ROOT
        / "tests/test_nodi_package_c_sidewall_simulation_assumption_input_workspace.py",
        "legacy_workspace_source": PROJECT_ROOT
        / "nodi_simulator/sidewall_real_evidence_input_workspace.py",
        "legacy_workspace_builder": PROJECT_ROOT
        / "tools/audits/build_nodi_package_c_sidewall_real_evidence_input_workspace.py",
    }
...
def _source_manifest_workspace_rows(*, create_missing_targets: bool) -> list[dict[str, Any]]:
    ...
    for spec in source_manifest_specs():
        ...
        if not path.exists():
            ...
            status = "source_manifest_simulation_assumption_rows_present_not_rewritten_by_workspace"
        else:
            status = "source_manifest_header_only_ready_no_simulation_assumption_rows"
        rows.append(
            {
                "workspace_row_id": f"SIMULATION-ASSUMPTION-SOURCE-MANIFEST-{spec['source_manifest_branch']}",
                "workspace_version": SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_VERSION,
                ...
                "simulation_assumption_current": False,
                ...
                "hard_fail_if": "source_manifest_header_only_rows_counted_as_claim_inputs",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    return rows
...
def build_payload(*, create_missing_targets: bool) -> dict[str, Any]:
    ...
    target_blocked = sum(
        ...
        not in {TARGET_HEADER_ONLY_STATUS, TARGET_SIMULATION_ROWS_PRESENT_STATUS}
    ...
    source_manifest_blocked = sum(
        row["target_validation_status"]
        not in {
            "source_manifest_header_only_ready_no_simulation_assumption_rows",
            "source_manifest_simulation_assumption_rows_present_not_rewritten_by_workspace",
        }
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    s = payload["summary"]
    failures: list[str] = []
    if s["disposition"] not in {DISPOSITION, SIMULATION_ROWS_DISPOSITION}:
...
def write_outputs(payload: dict[str, Any]) -> list[Path]:
    ...
    outputs = {
        ...
        "master_report": REPORT_DIR / f"582_{PREFIX}_20260701.md",
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# NODI Package C Sidewall Simulation Assumption Input Workspace",
            "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_sidewall_simulation_assumption_input_workspace:
        parser.error(
            "--confirm-sidewall-simulation-assumption-input-workspace is required"
        )
    payload = build_payload(create_missing_targets=True)
    failures = validate_payload(payload)
    if failures:
        print("BLOCKED_NODI_PACKAGE_C_SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE")
