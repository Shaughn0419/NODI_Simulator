...
PREFIX = "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH"
ARTIFACT_ID = (
    "PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_20260701"
)
DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_READY_NOT_CLAIM_READY"
)
BLOCKED_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_FAIL_CLOSED"
)
...
LEDGER_LANES = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_PROMOTION_LANE_ROWS_20260701.csv"
)
POLICY_STATUS = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_STATUS_20260701.json"
)
POLICY_ROWS = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_POLICY_ROWS_20260701.csv"
)
POLICY_BLOCKERS = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_BLOCKER_ROWS_20260701.csv"
)

ALLOWED_USE = (
    "route/yield/detection assembly after detector/blank transfer intake;"
...
SOURCE_FILES = {
    "integrated_detector_blank_transfer_status": LEDGER_STATUS,
    "integrated_detector_blank_transfer_lanes": LEDGER_LANES,
    "route_policy_detector_blank_transfer_status": POLICY_STATUS,
    "route_policy_detector_blank_transfer_rows": POLICY_ROWS,
    "route_policy_detector_blank_transfer_blockers": POLICY_BLOCKERS,
    ...
    "route_yield_detection_assembly_detector_blank_transfer_builder": PROJECT_ROOT
    / "tools/audits/build_nodi_package_c_sidewall_route_yield_detection_assembly_detector_blank_transfer_refresh.py",
    "route_yield_detection_assembly_detector_blank_transfer_builder_tests": PROJECT_ROOT
    / "tests/test_nodi_package_c_sidewall_route_yield_detection_assembly_detector_blank_transfer_refresh.py",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build route/yield/detection assembly after detector/blank transfer refresh."
    )
    parser.add_argument(
        "--confirm-sidewall-route-yield-detection-assembly-detector-blank-transfer-refresh",
        action="store_true",
    )
    return parser
...
def release_scoped_path(path: str) -> bool:
    ...
    return (
        ...
        or path.startswith(
            ...
            "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_"
        )
        or path
        == "reports/550_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
    )
...
def dirty_context_rows() -> list[dict[str, str]]:
    ...
    for line in git_status_lines():
        ...
        if path in STALE_POST_RC2_PATHS:
            ...
            classification = "assembly_detector_blank_transfer_build_edit"
            release_decision = "included_in_commit_scope_before_publish"
        elif path.startswith(
            "reports/joint_interface_20260701/"
            "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_"
        ) or path == (
            "reports/550_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_"
            "DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
        ):
            classification = "assembly_detector_blank_transfer_output"
            release_decision = "included_or_rewritten_by_assembly_detector_blank_transfer"
        elif release_scoped_path(path):
            classification = "release_scoped_dirty_blocker"
            release_decision = "blocks_assembly_detector_blank_transfer"
        else:
            classification = "non_release_dirty_context"
            release_decision = "ignored_for_assembly_detector_blank_transfer_not_source_locked"
        rows.append(
            {
                "path": path,
                "git_status": line[:2],
...
def build_payload() -> dict[str, Any]:
    ...
    assembly_rows, branch_rows = build_route_yield_detection_assembly(
        read_csv_rows(LEDGER_LANES),
        read_csv_rows(POLICY_ROWS),
        read_csv_rows(POLICY_BLOCKERS),
    )
    ...
    transfer_branch_rows = sum(
        row["branch_name"] == "sidewall_detector_blank_transfer_validation"
        and row["branch_status"]
    ...
    status = (
        ...
        == "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_READY_NOT_CLAIM_READY"
        and len(assembly_dicts) == 2
        and len(branch_dicts) == 8
        and geometry_families == ["ideal_rectangle", "trapezoid_tapered_sidewalls"]
        and transfer_branch_rows == 2
        and all(row["assembly_status"] == ASSEMBLY_NOT_CLAIM_READY_STATUS for row in assembly_dicts)
        and all(row["ready_input_lane_count"] == 3 for row in assembly_dicts)
        and all(row["candidate_context_lane_count"] == 3 for row in assembly_dicts)
        and all(row["missing_or_blocked_lane_count"] == 3 for row in assembly_dicts)
    ...
    summary: dict[str, Any] = {
        ...
        "transfer_intake_waiting_branch_rows": transfer_branch_rows,
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    checks = {
        ...
        "transfer branch rows": summary["transfer_intake_waiting_branch_rows"] == 2,
        "ready input lanes": summary["ready_input_lane_count_total"] == 6,
        "candidate context lanes": summary["candidate_context_lane_count_total"] == 6,
        "missing lanes": summary["missing_or_blocked_lane_count_total"] == 6,
        "next branch current": (
...
def write_outputs(payload: dict[str, Any]) -> list[Path]:
    ...
    public_report = (
        REPORT_DIR
        / "550_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
    )
...
def report_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# NODI Package C Sidewall Route/Yield/Detection Assembly Detector/Blank Transfer Refresh",
            "",
            ...
            f"- Transfer-intake waiting branch rows: `{s['transfer_intake_waiting_branch_rows']}`.",
        ...
        rows.append(
            {
                ...
                "policy_impact": "assembly_detector_blank_transfer_refresh_not_claim",
                "allowed_use": ALLOWED_USE,
                "blocked_use": BLOCKED_USE,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_sidewall_route_yield_detection_assembly_detector_blank_transfer_refresh:
        parser.error(
            "--confirm-sidewall-route-yield-detection-assembly-detector-blank-transfer-refresh is required"
        )
    payload = build_payload()
    failures = validate_payload(payload)
    if failures:
        print("BLOCKED_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_ASSEMBLY_DETECTOR_BLANK_TRANSFER_REFRESH")
