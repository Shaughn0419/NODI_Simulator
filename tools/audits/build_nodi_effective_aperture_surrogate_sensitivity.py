...
def _default_output_dir() -> Path:
    ...
    return Path("results/audits") / f"nodi_effective_aperture_runner_launch_{stamp}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "NODI_EFFECTIVE_APERTURE_SURROGATE_SENSITIVITY runner. This does not "
            "execute the runner, run smoke, run NODI/COMSOL, or generate production rows."
        )
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_write_launch_plan:
        parser.error(
            "refusing to write EAS runner launch plan without --confirm-write-launch-plan"
        )

    plan = write_effective_aperture_runner_launch_plan(args.output_dir)
    print(
        "NODI_EFFECTIVE_APERTURE_SURROGATE_RUNNER: "
        f"{plan['runner_implementation_status']}"
    )
