...
PREFIX = "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH"
ARTIFACT_ID = (
    "PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_20260701"
)
DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_READY_NOT_CLAIM_READY"
)
BLOCKED_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_FAIL_CLOSED"
)
...
ALLOWED_USE = (
    "route/yield/detection readiness policy after detector/blank transfer intake;"
...
SOURCE_FILES = {
    "integrated_detector_blank_transfer_status": BASE_LEDGER_STATUS,
    "integrated_detector_blank_transfer_lanes": BASE_LEDGER_LANES,
    ...
    "route_policy_detector_blank_transfer_builder": PROJECT_ROOT
    / "tools/audits/build_nodi_package_c_sidewall_route_yield_detection_policy_detector_blank_transfer_refresh.py",
    "route_policy_detector_blank_transfer_builder_tests": PROJECT_ROOT
    / "tests/test_nodi_package_c_sidewall_route_yield_detection_policy_detector_blank_transfer_refresh.py",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build route/yield/detection policy after detector/blank transfer refresh."
    )
    parser.add_argument(
        "--confirm-sidewall-route-yield-detection-policy-detector-blank-transfer-refresh",
        action="store_true",
    )
    return parser
...
def release_scoped_path(path: str) -> bool:
    ...
    return (
        ...
        or path.startswith(
            ...
            "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_"
        )
        or path
        == "reports/549_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
    )
...
def dirty_context_rows() -> list[dict[str, str]]:
    ...
    for line in git_status_lines():
        ...
        if path in STALE_POST_RC2_PATHS:
            ...
            classification = "route_policy_detector_blank_transfer_build_edit"
            release_decision = "included_in_commit_scope_before_publish"
        elif path.startswith(
            "reports/joint_interface_20260701/"
            "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH_"
        ) or path == (
            "reports/549_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_"
            "DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
        ):
            classification = "route_policy_detector_blank_transfer_output"
            release_decision = "included_or_rewritten_by_route_policy_detector_blank_transfer"
        elif release_scoped_path(path):
            classification = "release_scoped_dirty_blocker"
            release_decision = "blocks_route_policy_detector_blank_transfer"
        else:
            classification = "non_release_dirty_context"
            release_decision = "ignored_for_route_policy_detector_blank_transfer_not_source_locked"
        rows.append(
            {
                "path": path,
                "git_status": line[:2],
...
def build_payload() -> dict[str, Any]:
    ...
    policy_rows, blocker_rows = build_route_yield_detection_policy_rows(
        read_csv_rows(BASE_LEDGER_LANES)
    )
    ...
    status = (
        ...
        and detector_transfer_rows == 2
        and blank_transfer_rows == 2
        and selected_ready_blockers == 2
        and all(row["primary_next_execution_block"] == "sidewall_detector_blank_transfer_validation" for row in policy_dicts)
    ...
    summary: dict[str, Any] = {
        ...
        "branch": git_branch(),
        "source_detector_blank_transfer_ledger_disposition": base_status.get(
            "disposition", ""
        ),
        ...
        "selected_annulus_input_ready_blocker_rows": selected_ready_blockers,
        "detector_transfer_intake_policy_rows": detector_transfer_rows,
        "blank_transfer_intake_policy_rows": blank_transfer_rows,
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    checks = {
        ...
        "selected ready blockers": summary["selected_annulus_input_ready_blocker_rows"] == 2,
        "detector transfer recognized": summary["detector_transfer_intake_policy_rows"] == 2,
        "blank transfer recognized": summary["blank_transfer_intake_policy_rows"] == 2,
...
def report_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# NODI Package C Sidewall Route/Yield/Detection Policy Detector/Blank Transfer Refresh",
            "",
            ...
            "- Route score, winner, yield, wet pass probability, and detection probability remain blocked.",
            "",
        ]
    )
        ...
        rows.append(
            {
                ...
                "policy_impact": "route_policy_detector_blank_transfer_refresh_not_claim",
                "allowed_use": ALLOWED_USE,
                "blocked_use": BLOCKED_USE,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_sidewall_route_yield_detection_policy_detector_blank_transfer_refresh:
        parser.error(
            "--confirm-sidewall-route-yield-detection-policy-detector-blank-transfer-refresh is required"
        )
    payload = build_payload()
    failures = validate_payload(payload)
    if failures:
        print("BLOCKED_NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_DETECTOR_BLANK_TRANSFER_REFRESH")
