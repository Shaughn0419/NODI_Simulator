...
PREFIX = "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH"
ARTIFACT_ID = "PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_20260701"
DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_READY_PREFLIGHT_ONLY"
)
BLOCKED_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_FAIL_CLOSED"
)
SELF_MANIFEST_SHA256 = "SELF_MANIFEST_NOT_SELF_HASHED_BY_DESIGN"
CLAIM_BOUNDARY = "promotion_ledger_route_policy_refresh_not_route_score_not_yield_not_detection"
...
ROUTE_POLICY_STATUS = (
    OUTPUT_DIR / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_STATUS_20260701.json"
)
ROUTE_POLICY_ROWS = (
    OUTPUT_DIR / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_POLICY_ROWS_20260701.csv"
)
ROUTE_POLICY_PROMOTION_UPDATE = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_PROMOTION_UPDATE_ROWS_20260701.csv"
)

ALLOWED_USE = "integrated promotion ledger route policy refresh;route blocker prioritization"
BLOCKED_USE = (
...
SOURCE_FILES = {
    ...
    "route_yield_detection_policy_status": ROUTE_POLICY_STATUS,
    "route_yield_detection_policy_rows": ROUTE_POLICY_ROWS,
    "route_yield_detection_policy_promotion_update": ROUTE_POLICY_PROMOTION_UPDATE,
    "ledger_route_policy_refresh_builder": PROJECT_ROOT
    / "tools/audits/build_nodi_package_c_sidewall_integrated_promotion_ledger_route_policy_refresh.py",
    "ledger_route_policy_refresh_builder_tests": PROJECT_ROOT
    / "tests/test_nodi_package_c_sidewall_integrated_promotion_ledger_route_policy_refresh.py",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh sidewall integrated promotion ledger with route policy evidence."
    )
    parser.add_argument(
        "--confirm-sidewall-integrated-promotion-ledger-route-policy-refresh",
        action="store_true",
    )
    return parser
...
def release_scoped_path(path: str) -> bool:
    ...
    return (
        ...
        or path.startswith(
            ...
            "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_"
        )
        or path
        == "reports/540_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_20260701.md"
    )
...
def dirty_context_rows() -> list[dict[str, str]]:
    ...
    for line in git_status_lines():
        ...
        if path in STALE_POST_RC2_PATHS:
            ...
            classification = "integrated_promotion_ledger_route_policy_refresh_build_edit"
            release_decision = "included_in_commit_scope_before_publish"
        elif path.startswith(
            "reports/joint_interface_20260701/"
            "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH_"
        ) or path == (
            "reports/540_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_"
            "ROUTE_POLICY_REFRESH_20260701.md"
        ):
            classification = "integrated_promotion_ledger_route_policy_refresh_output"
            release_decision = (
                "included_or_rewritten_by_integrated_promotion_ledger_route_policy_refresh"
            )
        elif release_scoped_path(path):
            classification = "release_scoped_dirty_blocker"
            release_decision = "blocks_integrated_promotion_ledger_route_policy_refresh"
        else:
            classification = "non_release_dirty_context"
            release_decision = (
                "ignored_for_integrated_promotion_ledger_route_policy_refresh_not_source_locked"
            )
        rows.append(
            {
                "path": path,
...
def promotion_update_row() -> dict[str, str]:
    rows = read_csv_rows(ROUTE_POLICY_PROMOTION_UPDATE)
    if len(rows) != 1:
        return {}
    return rows[0]
...
def refreshed_promotion_lane_rows() -> list[dict[str, str]]:
    rows = read_csv_rows(LEDGER_WET_LANES)
    update = promotion_update_row()
    policy_sha = sha256_file(ROUTE_POLICY_ROWS)
    policy_path = display_path(ROUTE_POLICY_ROWS)
    policy_disposition = "NODI_PACKAGE_C_SIDEWALL_ROUTE_YIELD_DETECTION_POLICY_READY_NOT_CLAIM_READY"
    for row in rows:
        if row.get("evidence_lane") != update.get("target_ledger_lane"):
            continue
        row["source_artifact"] = policy_path
        row["source_sha256"] = policy_sha
        row["source_disposition"] = policy_disposition
...
def refresh_delta_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    return [
        {
            "delta_id": f"ROUTE-POLICY-REFRESH-{row['route_candidate_id']}",
...
def build_payload() -> dict[str, Any]:
    ...
    route_policy_rows = read_csv_rows(ROUTE_POLICY_ROWS) if ROUTE_POLICY_ROWS.exists() else []
    integrated_policy = sum(
    ...
    status = (
        ...
        and len(route_policy_rows) == 2
        and len(deltas) == 2
        and integrated_policy == 2
    ...
    summary: dict[str, Any] = {
        ...
        "source_route_policy_disposition": route_policy_status.get("disposition", ""),
...
def report_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# NODI Package C Sidewall Integrated Promotion Ledger Route Policy Refresh",
            "",
            f"- Disposition: `{s['disposition']}`.",
            f"- Current head: `{s['current_head']}` on `{s['branch']}`.",
            f"- Refreshed promotion lane rows: `{s['refreshed_promotion_lane_rows']}`.",
            f"- Route-policy delta rows: `{s['route_policy_delta_rows']}`.",
            "- This refresh updates the integrated_route_ledger lanes with the 539 route/yield/detection policy.",
        ...
        rows.append(
            {
                ...
                "policy_impact": "route_policy_ledger_refresh_not_promotion",
                "allowed_use": ALLOWED_USE,
                "blocked_use": BLOCKED_USE,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_sidewall_integrated_promotion_ledger_route_policy_refresh:
        parser.error(
            "--confirm-sidewall-integrated-promotion-ledger-route-policy-refresh is required"
        )
    payload = build_payload()
    failures = validate_payload(payload)
    if failures:
        print("BLOCKED_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_ROUTE_POLICY_REFRESH")
