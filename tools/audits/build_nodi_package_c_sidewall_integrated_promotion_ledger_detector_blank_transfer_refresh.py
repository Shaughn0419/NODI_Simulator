...
PREFIX = "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH"
ARTIFACT_ID = (
    "PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_20260701"
)
DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_READY_PREFLIGHT_ONLY"
)
BLOCKED_DISPOSITION = (
    "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_FAIL_CLOSED"
)
SELF_MANIFEST_SHA256 = "SELF_MANIFEST_NOT_SELF_HASHED_BY_DESIGN"
CLAIM_BOUNDARY = (
    "promotion_ledger_detector_blank_transfer_refresh_not_detection_probability"
)
...
TRANSFER_INTAKE_PROMOTION_UPDATE = (
    OUTPUT_DIR
    / "NODI_PACKAGE_C_SIDEWALL_DETECTOR_BLANK_TRANSFER_INTAKE_PROMOTION_UPDATE_ROWS_20260701.csv"
)

ALLOWED_USE = (
    "integrated promotion ledger detector/blank transfer refresh;"
    "detector blank transfer intake handoff"
)
BLOCKED_USE = (
...
SOURCE_FILES = {
    "base_integrated_wet_observation_status": BASE_LEDGER_STATUS,
    "base_integrated_wet_observation_lanes": BASE_LEDGER_LANES,
    ...
    "detector_blank_transfer_intake_promotion_update": TRANSFER_INTAKE_PROMOTION_UPDATE,
    "ledger_detector_blank_transfer_refresh_builder": PROJECT_ROOT
    / "tools/audits/build_nodi_package_c_sidewall_integrated_promotion_ledger_detector_blank_transfer_refresh.py",
    "ledger_detector_blank_transfer_refresh_tests": PROJECT_ROOT
    / "tests/test_nodi_package_c_sidewall_integrated_promotion_ledger_detector_blank_transfer_refresh.py",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh integrated promotion ledger with detector/blank transfer intake."
    )
    parser.add_argument(
        "--confirm-sidewall-integrated-promotion-ledger-detector-blank-transfer-refresh",
        action="store_true",
    )
    return parser
...
def release_scoped_path(path: str) -> bool:
    ...
    return (
        ...
        or path.startswith(
            ...
            "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_"
        )
        or path
        == "reports/548_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
    )
...
def dirty_context_rows() -> list[dict[str, str]]:
    ...
    for line in git_status_lines():
        ...
        if path in STALE_POST_RC2_PATHS:
            ...
            classification = "integrated_detector_blank_transfer_refresh_build_edit"
            release_decision = "included_in_commit_scope_before_publish"
        elif path.startswith(
            "reports/joint_interface_20260701/"
            "NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH_"
        ) or path == (
            "reports/548_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_"
            "DETECTOR_BLANK_TRANSFER_REFRESH_20260701.md"
        ):
            classification = "integrated_detector_blank_transfer_refresh_output"
            release_decision = (
                "included_or_rewritten_by_integrated_detector_blank_transfer_refresh"
            )
        elif release_scoped_path(path):
            classification = "release_scoped_dirty_blocker"
            release_decision = "blocks_integrated_detector_blank_transfer_refresh"
        else:
            classification = "non_release_dirty_context"
            release_decision = (
                "ignored_for_integrated_detector_blank_transfer_refresh_not_source_locked"
            )
        rows.append(
            {
                "path": path,
...
def promotion_update_rows() -> list[dict[str, str]]:
    return read_csv_rows(TRANSFER_INTAKE_PROMOTION_UPDATE)


def refreshed_promotion_lane_rows() -> list[dict[str, str]]:
    rows = read_csv_rows(BASE_LEDGER_LANES)
    updates = {row["target_ledger_lane"]: row for row in promotion_update_rows()}
    matrix_sha = sha256_file(TRANSFER_INTAKE_MATRIX_ROWS)
    matrix_path = display_path(TRANSFER_INTAKE_MATRIX_ROWS)
    matrix_disposition = (
...
def refresh_delta_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    target_lanes = {row["target_ledger_lane"] for row in promotion_update_rows()}
    return [
        {
            "delta_id": f"DETECTOR-BLANK-TRANSFER-REFRESH-{row['route_candidate_id']}-{row['evidence_lane']}",
...
def build_payload() -> dict[str, Any]:
    base_status = load_json(BASE_LEDGER_STATUS)
    ...
    status = (
        ...
        and base_status.get("disposition")
    ...
    summary: dict[str, Any] = {
        ...
        "branch": git_branch(),
        "source_base_ledger_disposition": base_status.get("disposition", ""),
        "source_transfer_intake_disposition": transfer_status.get("disposition", ""),
        "refreshed_promotion_lane_rows": len(lanes),
        "detector_blank_transfer_delta_rows": len(deltas),
        "detector_transfer_intake_rows": counts["detector_transfer"],
        "blank_transfer_intake_rows": counts["blank_transfer"],
        "formal_qch_lane_rows_retained": counts["qch"],
        "pressure_flow_validation_rows_retained": counts["pressure"],
        "selected_annulus_panel_rows_retained": counts["selected"],
        "wet_observation_intake_rows_retained": counts["wet"],
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    checks = {
        ...
        "four detector blank transfer deltas": summary["detector_blank_transfer_delta_rows"] == 4,
        "detector transfer rows": summary["detector_transfer_intake_rows"] == 2,
        "blank transfer rows": summary["blank_transfer_intake_rows"] == 2,
        "qch retained": summary["formal_qch_lane_rows_retained"] == 2,
        "pressure retained": summary["pressure_flow_validation_rows_retained"] == 2,
        "selected retained": summary["selected_annulus_panel_rows_retained"] == 2,
        "wet retained": summary["wet_observation_intake_rows_retained"] == 2,
...
def report_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# NODI Package C Sidewall Integrated Promotion Ledger Detector/Blank Transfer Refresh",
            "",
            f"- Disposition: `{s['disposition']}`.",
            f"- Current head: `{s['current_head']}` on `{s['branch']}`.",
            f"- Refreshed promotion lane rows: `{s['refreshed_promotion_lane_rows']}`.",
            f"- Detector/blank transfer delta rows: `{s['detector_blank_transfer_delta_rows']}`.",
            "- Detector and blank lanes now point to the 547 transfer-intake matrix.",
        ...
        rows.append(
            {
                ...
                "policy_impact": "integrated_detector_blank_transfer_refresh_not_claim",
                "allowed_use": ALLOWED_USE,
                "blocked_use": BLOCKED_USE,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_sidewall_integrated_promotion_ledger_detector_blank_transfer_refresh:
        parser.error(
            "--confirm-sidewall-integrated-promotion-ledger-detector-blank-transfer-refresh is required"
        )
    payload = build_payload()
    failures = validate_payload(payload)
    if failures:
        print("BLOCKED_NODI_PACKAGE_C_SIDEWALL_INTEGRATED_PROMOTION_LEDGER_DETECTOR_BLANK_TRANSFER_REFRESH")
