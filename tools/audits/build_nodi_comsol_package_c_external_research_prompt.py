...
def context_rows() -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            ...
            "details": "u marginal L1 candidate pass but proof gap; see threshold table for x-local metric too.",
            "github_url": blob_url(THRESHOLD_TABLE),
            "claim_boundary": CLAIM_BOUNDARY,
        },
        {
            "context_id": "one_wall_gap",
            "context_value": row_by_metric.get("max_one_wall_positive_control_ks", {}).get(
                "observed_value",
                "",
            ),
            "details": "One-wall KS candidate pass <=0.02 but not proof target <=0.01.",
            "github_url": blob_url(THRESHOLD_TABLE),
            "claim_boundary": CLAIM_BOUNDARY,
        },
        {
