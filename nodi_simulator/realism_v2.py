"""Package-local instrument-aware EV/NODI realism v2 sidecars.
...
def _load_sibling_module(module_name: str) -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        f"_nodi_simulator_direct_{module_name}",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load sibling module {module_name!r} from {module_path}")
...
DEFAULT_REDUCED_GRID_R3A_DIR = PROJECT_ROOT / "results" / "ev_nodi_realism_v2_reduced_grid_R3a"
DEFAULT_UNCERTAINTY_R3B_DIR = PROJECT_ROOT / "results" / "ev_nodi_realism_v2_uncertainty_R3b"
...
DEFAULT_REVISED_R4_RERUN_DIR = PROJECT_ROOT / "results" / "ev_nodi_realism_v2_revised_R4_rerun"
DEFAULT_R4_2_ADJUDICATION_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R4_2_main660_nearwall_mesh_adjudication"
)
DEFAULT_R5_FULL_GRID_V2_DIR = PROJECT_ROOT / "results" / "ev_nodi_realism_v2_full_grid_R5_v2"
DEFAULT_R5_1_INTERPRETATION_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R5_1_route_role_stability_interpretation"
)
DEFAULT_R5_2_BOUNDED_SCENARIO_PRIOR_AUDIT_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R5_2_bounded_scenario_prior_audit"
)
DEFAULT_R5_3_ROUTE_PRIOR_MODEL_REVISION_AUDIT_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R5_3_route_prior_model_revision_audit"
)
...
DEFAULT_R7_ROUTE_PRIOR_MECHANISTIC_DECOMPOSITION_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R7_route_prior_mechanistic_decomposition_audit"
)
DEFAULT_R7_1_OPERATOR_ARTIFACT_VALIDATION_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R7_1_operator_artifact_validation_protocol"
)
DEFAULT_R7_2_OPERATOR_ARTIFACT_GAP_REGISTER_DIR = (
    PROJECT_ROOT / "results" / "ev_nodi_realism_v2_R7_2_operator_artifact_gap_register"
)
...
def run_manifest_provenance_checksum(path: str | Path) -> str:
    """Return a stable checksum for run manifests used as provenance inputs.

    ``created_at`` records when a historical writer was re-run. Downstream plans
    should freeze scientific and governance content, not fail only because that
    volatile timestamp changed.
    """
    manifest = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(manifest, dict):
        raise ValueError("run_manifest.json must contain a JSON object")
    for key in RUN_MANIFEST_VOLATILE_FIELDS:
...
def _validate_run_manifest_checksum_kind(
    provenance: Mapping[str, Any],
    required_fields: set[str],
    label: str,
) -> None:
    if not any(field.endswith("_run_manifest_checksum") for field in required_fields):
        return
    expected = RUN_MANIFEST_PROVENANCE_CHECKSUM_KIND
    actual = provenance.get("run_manifest_checksum_kind")
    if actual == expected or actual in RUN_MANIFEST_LEGACY_CHECKSUM_KINDS:
        return
    if actual != expected:
        raise ValueError(
            f"{label} run manifest checksum kind mismatch: expected {expected!r}, got {actual!r}"
        )
...
R3B_EFFECT_DELTA_CONVENTION = "median_log_score_ratio_over_particles_seeds_prior_samples"
R3B_ROUTE_PANEL = (
...
R5_2_EXISTING_R5_AUDIT_ROW_CAP = (
    R5_2_AUDIT_ROUTE_COUNT * R5_V1_SOURCE_PARTICLE_NAME_COUNT * R5_NAMED_SCENARIO_BUNDLE_COUNT
)
...
R7_DERIVED_CANDIDATE_ROW_CAP = R7_ROUTE_PRIOR_SOURCE_ROW_CAP * R7_MAX_MECHANISTIC_CANDIDATE_COUNT
...
V2_NO_MEASURED_DATA_CLOSURE_SCHEMA_VERSION = "V2_no_measured_data_closure_plan_v1"
...
def load_json_yaml(name_or_path: str | Path) -> dict[str, Any]:
    """Load a config file that is deliberately strict JSON-compatible YAML."""
...
def registry_artifact_ids(registry: dict[str, Any] | None = None) -> set[str]:
    payload = (
        registry if registry is not None else load_json_yaml("calibration_artifact_registry.yaml")
    )
...
def canonical_instrument_path_id(path_id: str, path_schema: dict[str, Any] | None = None) -> str:
    schema = path_schema if path_schema is not None else detector_path_schema()
...
def validate_detector_path_schema_maps_to_state_machine(
    ...
    machine = (
        ...
        else load_json_yaml("detector_connection_state_machine.yaml")
    )
...
def local_irradiance_W_per_m2(*, P_probe_W: float, p_beam_at_particle_per_m2: float) -> float:
    if P_probe_W < 0:
...
def bfp_roi_intensity_operator(
    *,
    ...
    jac = direction_cosine_jacobian(safe_u, safe_v) if apply_jacobian else np.ones_like(weight)
...
def scenario_detector_unit_sidecar(
    *,
    ...
    detector_transfer_unlocked = artifact_unlocks_detector_transfer(detector_transfer_artifact)
    ...
    if calibrated_snr_unlocked:
        ...
        if measured_blank_artifact_id:
            calibration_blocker_parts.append(
                "blank_artifact_not_valid_for_measured_blank" if not measured_blank_unlocked else ""
            )
        else:
    ...
    return {
        ...
        "shot_noise_fraction": float(terms["shot_noise_A2_per_Hz"])
        / max(
            1.0e-300,
            float(terms["shot_noise_A2_per_Hz"])
            + float(terms["Johnson_noise_A2_per_Hz_equivalent"])
        ...
        "RIN_noise_fraction": float(terms["RIN_noise_A2_per_Hz"])
        / max(
            1.0e-300,
            float(terms["shot_noise_A2_per_Hz"])
            + float(terms["Johnson_noise_A2_per_Hz_equivalent"])
...
def zero_event_upper_bound_per_min(
    *, observed_zero_duration_s: float, confidence: float = 0.95
) -> float:
...
def blank_false_positive_sidecar(
    *,
    ...
    gaussian_fp_min = (
        gaussian_tail_probability(threshold_sigma, "two_sided") * effective_N_per_s * 60.0
    )
    magnitude_fp_min = (
        rayleigh_magnitude_tail_probability(threshold_sigma) * effective_N_per_s * 60.0
    )
    return {
        "trace_hierarchy": (
...
def validate_uncertainty_R3b_prior_table(
    ...
    for row in rows:
        ...
        if correlation_transform not in R3B_ALLOWED_CORRELATION_TRANSFORMS:
            raise ValueError(f"Unknown R3b correlation_transform: {correlation_transform}")
    ...
    for row in rows:
        ...
        if group_counts[group] > 1 and not row.get("correlation_transform"):
            raise ValueError(f"R3b grouped prior lacks correlation_transform: {group}")
    return payload
...
def R3b_group_component_log_multiplier_equivalence_check(
    ...
    *,
    ...
    errors = {route: abs(direct[route] - reconstructed[route]) for route in direct}
...
def _R3b_particle_class_flags(particle: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "is_small_ev": 1.0
        if particle_id in {"EV50_nominal", "EV70_nominal", "EV70_lowRI"}
        else 0.0,
...
def R3b_group_component_log_multipliers(
    *,
    ...
    bfp_multiplier = (
        ...
        * math.exp(
            -(factor_values["leakage_floor"] - 0.0001) * (110.0 * (weak + 0.45 * selected - 0.12))
        )
    )
    detector_multiplier = (
        ...
        ** (0.28 * (features["geometry_factor"] - 1.0) + 0.35 * weak)
        / (
            (factor_values["input_noise_scale"] / 1.0)
            ** (0.22 * (weak - 0.05) + 0.20 * (features["geometry_factor"] - 1.0))
        )
        / (
        ...
        / (
            (factor_values["ADC_quantization_scale"] / 1.0)
            ** (0.25 * (weak + 0.30 * selected - 0.10))
        )
    )
    wall_multiplier = (
        ...
        * (
            (factor_values["count_rate_proxy_factor"] / 1.0)
            ** (0.20 * (features["geometry_factor"] - 1.0))
        )
    )
    blank_multiplier = (
        (5.0 / factor_values["blank_threshold_sigma"]) ** (0.55 * (weak + 0.25 * selected - 0.08))
        * (factor_values["independent_samples_per_s"] / 20.0)
        ** (0.08 * (weak + 0.20 * selected - 0.06))
        * (0.5 / factor_values["colored_noise_correlation_time_s"])
        ** (0.07 * (weak + 0.20 * selected - 0.06))
        * (0.001 / factor_values["rare_burst_rate_prior"])
        ** (0.035 * (weak + 0.20 * selected - 0.06))
    )
    if wavelength_nm == 404:
        ...
        thermal_multiplier = math.exp(-0.18 * max(0.0, thermal_stress - 1.0) * (1.0 + 0.35 * short))
    else:
...
def _R3b_adjusted_scenario(factor_values: dict[str, float], wavelength_nm: int) -> dict[str, Any]:
    ...
    scenario["blank_independent_samples_per_s"] = float(factor_values["independent_samples_per_s"])
...
def estimate_representative_full_wave_R4_cost(
    *,
    ...
    solver_case_count = (
        n_routes * n_particles * n_interface_states * n_polarization_states * n_mesh_levels
    )
    under_cap = (
...
def validate_representative_full_wave_R4_plan(
    ...
    route_keys = {
        (int(row["wavelength_nm"]), int(row["width_nm"]), int(row["depth_nm"])) for row in routes
    }
    ...
    missing_solver_fields = [
        ...
        if field not in solver_contract
        or solver_contract[field] is None
        or solver_contract[field] == ""
    ]
    ...
    observable_ids = {
        str(row["observable_id"]) for row in payload.get("observables", []) if isinstance(row, dict)
    }
...
def validate_R4_route_model_revision_plan(
    ...
    if sign_contract["delta_intensity_identity"] != (
        "|E_ref + E_sca|^2 - |E_ref|^2 = 2*Re(E_ref*conj(E_sca)) + |E_sca|^2"
    ):
...
def _R4_main_660_global_flip_failure_groups() -> dict[tuple[str, str, str], int]:
    observable_path = DEFAULT_REPRESENTATIVE_FULL_WAVE_R4_DIR / ("full_wave_observable_summary.csv")
...
def validate_R4_revised_rerun_plan(
    ...
    if evidence.get("best_allowed_convention_id") != ("global_full_wave_cross_term_sign_flip"):
    ...
    if convention.get("canonical_delta_P_NODI_identity") != (
        "Delta_P_NODI = |E_ref + E_sca|^2 - |E_ref|^2 = |E_sca|^2 + 2*Re(E_ref*conj(E_sca))"
    ):
        raise ValueError("R4 revised rerun cross-term identity mismatch")
    if "full_wave_cross_term_signed_W" not in str(convention.get("polarity_mapping_questions", [])):
...
def estimate_R4_2_adjudication_cost(
    *,
    ...
    solver_case_count = (
        n_routes * n_particles * n_interface_states * n_polarization_states * n_new_mesh_levels
    )
    ...
    return {
        ...
        "solver_case_count": solver_case_count,
        "max_R4_2_solver_cases_before_review": (MAX_R4_2_ADJUDICATION_SOLVER_CASES_BEFORE_REVIEW),
...
def validate_R4_2_adjudication_plan(
    ...
    if (
        set(solver_scope.get("validation_grade_mesh_levels", []))
        != R4_2_ADJUDICATION_VALIDATION_MESH_LEVEL_ROLES
    ):
...
def validate_R5_full_grid_v2_plan(
    ...
    if set(provenance.get("required_checksum_fields", [])) != (R5_REQUIRED_SOURCE_CHECKSUM_FIELDS):
        raise ValueError("R5 required checksum field set mismatch")
    _validate_run_manifest_checksum_kind(
        provenance,
        R5_REQUIRED_SOURCE_CHECKSUM_FIELDS,
        "R5",
    )
...
def validate_R5_scenario_bundle_manifest(
    ...
    code_rows = {str(row["scenario_id"]): row for row in anchor_smoke_scenario_bundles()}
    for row in scenario_rows:
        ...
        for key in R5_REQUIRED_SCENARIO_BUNDLE_FIELDS:
            ...
            if isinstance(value, int | float):
                if not math.isclose(float(value), float(code_value), rel_tol=0.0, abs_tol=1e-15):
                    raise ValueError(
                        f"R5 scenario manifest/code mismatch: {row['scenario_id']} {key}"
                    )
...
def validate_R5_1_route_role_stability_plan(
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_R5_checksum_fields", [])) != (R5_1_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R5_2_bounded_scenario_prior_audit_plan(
    ...
    if int(design.get("audit_existing_R5_source_row_cap", -1)) != (R5_2_EXISTING_R5_AUDIT_ROW_CAP):
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R5_2_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R5_3_route_prior_model_revision_plan(
    ...
    if set(candidates.get("forbidden_prior_families", [])) != (R5_3_FORBIDDEN_PRIOR_FAMILIES):
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R5_3_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R6_route_prior_sensitivity_plan(
    ...
    if carry.get("audit_execution_type") != ("bounded_existing_R5_artifact_prior_model_audit_only"):
    ...
    if carry.get("selected_candidate_prior_id") != ("global_width_quadratic_regularization"):
        raise ValueError("R6 must carry forward the R5.3 selected width candidate")
    if carry.get("selected_candidate_prior_family") != ("global_width_depth_regularization_family"):
    ...
    if int(design.get("max_R6_derived_candidate_rows", -1)) != (R6_DERIVED_CANDIDATE_ROW_CAP):
    ...
    if comparator.get("primary_pass_fail_comparator") != ("candidate_adjusted_locked_main_660"):
    ...
    if int(sensitivity.get("candidate_prior_count", -1)) != len(R6_CANDIDATE_PRIOR_REGISTRY):
    ...
    if set(sensitivity.get("forbidden_prior_families", [])) != (R6_FORBIDDEN_PRIOR_FAMILIES):
    ...
    if {row.get("candidate_prior_id") for row in registry} != (R6_REQUIRED_CANDIDATE_PRIOR_IDS):
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R6_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R7_route_prior_mechanistic_decomposition_plan(
    ...
    payload = plan if plan is not None else load_R7_route_prior_mechanistic_decomposition_plan()
    ...
    if int(carry.get("derived_candidate_rows_evaluated", -1)) != (R6_DERIVED_CANDIDATE_ROW_CAP):
    ...
    if carry.get("at_least_two_nearby_low_dimensional_candidates_explain_warning") is not True:
    ...
    if (
        float(carry.get("width_quad_900_main660_score_retention_fraction", 0.0))
    ...
    if int(design.get("max_R7_derived_candidate_rows", -1)) != (R7_DERIVED_CANDIDATE_ROW_CAP):
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R7_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R7_1_operator_artifact_validation_plan(
    ...
    if payload.get("selected_next_stage_lane") != ("operator_artifact_gap_register_plan_only"):
    ...
    if set(design.get("required_evidence_module_ids", [])) != (R7_1_REQUIRED_EVIDENCE_MODULE_IDS):
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R7_1_REQUIRED_PROVENANCE_FIELDS):
...
def validate_R7_2_operator_artifact_gap_register_plan(
    ...
    provenance = payload.get("provenance_freeze", {})
    if set(provenance.get("required_checksum_fields", [])) != (R7_2_REQUIRED_PROVENANCE_FIELDS):
...
def validate_v2_no_measured_data_closure_plan(
    ...
    if payload.get("prior_gate") != ("PASS_R7_2_RESULTS_PREPARE_V2_NO_MEASURED_DATA_CLOSURE_ONLY"):
...
def build_run_manifest(
    *,
    ...
    manifest = {
        ...
        "R3a_reduced_grid_named_bundle_survey_run": (R3a_reduced_grid_named_bundle_survey_run),
        "R3b_uncertainty_expansion_run": R3b_uncertainty_expansion_run,
        "R4_representative_full_wave_validation_run": (R4_representative_full_wave_validation_run),
...
def validate_run_manifest(manifest: dict[str, Any]) -> None:
    ...
    for field in checksum_fields:
        ...
        if value != "unavailable" and not _is_sha256_hex(value):
...
def run_anchor_smoke(
    ...
    *,
        ...
        for particle in particles:
            ...
            for scenario in scenarios:
                ...
                blank = blank_false_positive_sidecar(
                    ...
                    independent_samples_per_s=float(scenario["blank_independent_samples_per_s"]),
                    colored_noise_correlation_time_s=float(
                ...
                raw_snr = detector["scenario_detector_SNR"] * float(scenario["daq_snr_factor"])
                effective_snr = (
                    raw_snr
                ...
                for seed in seeds:
                    ...
                    adaptive_event_count_will_not_clear_gate = (
                        low_detectability_prior and not statistical_precision_rerun_recommended
                    )
                    ...
                    row = {
                        ...
                        "P_ref_ROI_W": bfp["P_ref_ROI_W"],
                        "P_ref_scale_independent_of_particle": True,
                        "reference_power_prior_scale_W": bfp["reference_power_prior_scale_W"],
                        "P_ref_ROI_W_unit": "W",
                        "P_sca_ROI_W": mie["P_sca_ROI_W"],
                        "P_sca_ROI_W_unit": "W",
                        "P_cross_ROI_W": bfp["P_cross_ROI_W"],
    ...
    for (wavelength_nm, width_nm, depth_nm, particle_id), values in sorted(route_aggregate.items()):
...
def _correlation_pair(xs: list[float], ys: list[float]) -> float:
    ...
    def effectively_constant(arr: np.ndarray) -> bool:
        scale = max(float(np.mean(np.abs(arr))), float(np.max(np.abs(arr))), 1.0e-30)
        return float(np.std(arr)) <= 1.0e-12 * scale

    if effectively_constant(x_arr) or effectively_constant(y_arr):
        return float("nan")
...
def run_reduced_grid_R3a(
    ...
    *,
            ...
            for scenario in scenarios:
                ...
                for seed in seeds:
                    ...
                    if str(particle["particle_class"]) == "EV":
                        snr_by_role_scenario.setdefault((route_role, scenario_id), []).append(
    ...
    scenario_rows.sort(key=lambda row: row["mean_detectability_relative_prior_score"], reverse=True)
    ...
    r3_means = {key: float(np.mean(values)) for key, values in route_particle_values.items()}
    ...
    *,
    ...
    for route in route_rows:
        ...
        for particle in particles:
            ...
            nominal_cache[(wavelength_nm, width_nm, depth_nm, str(particle["particle_id"]))] = {
    ...
    for route in route_rows:
        ...
        for particle in particles:
            ...
            for prior_sample in prior_samples:
                ...
                adjusted_particle_factor = _R3b_adjusted_particle_factor(particle, factor_values)
                mie, bfp = _anchor_smoke_power(
                    wavelength_nm=wavelength_nm,
                    width_nm=width_nm,
                    depth_nm=depth_nm,
                ...
                thermal = thermal_404_sidecar(
                    wavelength_nm=wavelength_nm,
                    I_exc_W_per_m2=mie["I_inc_W_per_m2"],
                    alpha_medium_1_per_m=0.02 * float(factor_values["medium_absorption_scale"]),
                    medium_volume_m3=1.0e-18,
                    alpha_glass_1_per_m=0.05 * float(factor_values["glass_absorption_scale"]),
                    glass_volume_m3=1.0e-18,
                    particle_abs_cross_section_m2=0.05 * mie["Csca_total_m2"],
                    contaminant_abs_cross_section_m2=0.02 * mie["Csca_total_m2"],
                    filter_leakage_fraction=1.0e-4 * float(factor_values["filter_leakage_scale"]),
                )
                ...
                nominal_score = float(nominal_entry["nominal_detectability_relative_prior_score"])
                ...
                effective_snr = (
                    ...
                    * math.exp(
                        sum(
                            group_logs[group]
                            for group in ("detector_readout", "BFP_slit_alignment")
                        )
                    )
                    * thermal_gate_multiplier
                )
                ...
                for seed in seeds:
                    seed_adjustment = 1.0 + ((seed % 7) - 3) * 0.015
                    score = float(max(0.0, min(1.0, base_score_with_groups * seed_adjustment)))
                    ...
                    row = {
                        ...
                        "BFP_slit_alignment_log_multiplier": group_logs["BFP_slit_alignment"],
                        "detector_readout_log_multiplier": group_logs["detector_readout"],
                        "wall_PEG_flow_log_multiplier": group_logs["wall_PEG_flow"],
                        "blank_RIN_drift_log_multiplier": group_logs["blank_RIN_drift"],
                        "thermal_404_log_multiplier": group_logs["thermal_404"],
                        ...
                        "near_wall_event_fraction": factor_values["near_wall_event_fraction"],
                        "adsorption_loss_factor": factor_values["adsorption_loss_factor"],
                        "blank_threshold_sigma": factor_values["blank_threshold_sigma"],
                        "independent_samples_per_s": factor_values["independent_samples_per_s"],
    ...
    global_rows = [
        {
            "effect_delta_convention": R3B_EFFECT_DELTA_CONVENTION,
            "max_group_route_sensitive_index": overall_status["max_group_route_sensitive_index"],
            ...
            "global_scalar_dominated_stop_gate": (
                overall_status["route_sensitive_prior_status"] == "global_scalar_dominated"
            ),
            "claim_level": "relative_with_priors",
        }
    ]
    ...
    top_route = _first_row(route_summary_rows, "R3b route summary")
    report = (
    ...
    return {
        ...
        "route_sensitive_prior_status": overall_status["route_sensitive_prior_status"],
        "global_multiplier_dominance_index": overall_status["global_multiplier_dominance_index"],
        "R3b_uncertainty_expansion_run": True,
        "R4_representative_full_wave_validation_run": False,
        "R5_full_grid_v2_run": False,
    }
...
def _R4_solver_backend_status(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    available_modules = [module for module in candidate_modules if importlib.util.find_spec(module)]
...
def _R4_effective_complex_RI(material: dict[str, Any], wavelength_nm: int) -> complex:
    ...
    eps_eff = core_volume_fraction * core_n**2 + (1.0 - core_volume_fraction) * shell_n**2
    return complex(math.sqrt(eps_eff), 0.0)


def _R4_reference_power_scale_W(*, width_nm: int, depth_nm: int, scenario: dict[str, Any]) -> float:
    geometry_factor = max(0.1, (width_nm / 800.0) * (depth_nm / 1400.0))
    return (
        1.0e-8
        * geometry_factor
...
def _R4_channel_modal_green_solver_case(
    *,
    ...
    alpha_eff = modal_amplitude / (1.0 - modal_amplitude * green_self * k_medium * 1.0e-3)
    ...
    return {
        **payload,
        ...
        "source_normalization_record": json.dumps(contract["source_normalization"], sort_keys=True),
        ...
        "BFP_ROI_extraction_record": json.dumps(
            {
                "BFP_far_field_extraction_method": contract["BFP_far_field_extraction_method"],
...
def _R4_decision_label(
    *,
    ...
    if (
        not sign_preserved
        or abs(surrogate_delta_log) >= float(thresholds["surrogate_delta_log_demote_abs_min"])
    ...
    if (
        mesh_refined_delta_abs > float(thresholds["mesh_refined_delta_abs_max"])
        ...
        return "inconclusive_requires_plan_revision"
    if (
        abs(surrogate_delta_log) <= float(thresholds["surrogate_delta_log_confirm_abs_max"])
        and polarization_sensitivity_abs <= float(thresholds["polarization_sensitivity_abs_max"])
        and near_wall_stress_delta_abs <= float(thresholds["near_wall_stress_delta_abs_max"])
    ):
...
def run_representative_full_wave_R4(
    ...
    *,
    ...
    material_by_id = {str(row["particle_id"]): row for row in plan["particle_material_contract"]}
            ...
            for interface_state in interface_states:
                for polarization_state in polarization_states:
                    for mesh_level in mesh_levels:
                        ...
                        prior_sample_id = f"{interface_state}_{polarization_state}_{mesh_level}"
                        scenario_identity = make_scenario_identity(
                            scenario_id=scenario_id,
                        ...
                        surrogate_delta_log = _R4_log_abs_ratio(full_signal, surrogate_signal, eps)
                        if particle_id == "blank":
                            sign_preserved = True
                        else:
                        ...
                        thermal_sidecar_blocks = (
                            wavelength_nm == 404 and thermal["thermal_artifact_risk"] == "red"
                        )
                        decision_label = _R4_decision_label(
                            solver_backend_available=bool(
                                solver_status["solver_backend_available"]
                            ),
                            solver_case_completed=bool(solver_result["solver_case_completed"]),
                        ...
                        case_row = {
                            **common,
                            ...
                            "BFP_valid_uv_fraction": solver_result["BFP_valid_uv_fraction"],
                            ...
                            "BFP_ROI_extraction_record": solver_result["BFP_ROI_extraction_record"],
                            ...
                            "polarization_vector_definition": json.dumps(
                                contract["polarization_vector_definition"][polarization_state],
                                sort_keys=True,
                            ),
                            ...
                            "mesh_convergence_metric": contract["mesh_convergence_metric"],
                            "mesh_convergence_threshold": contract["mesh_convergence_threshold"],
                        ...
                        observable_row = {
                            **common,
                            ...
                            "thermal_sidecar_blocks_404_interpretation": (thermal_sidecar_blocks),
                        ...
                        bfp_rows.append(
                            {
                                **common,
                                ...
                                "pinhole_surrogate_ROI_signal_signed_W": (surrogate_signal * 0.82),
                                "BFP_jacobian_policy": contract["BFP_jacobian_policy"],
                                "same_operator_applied_to_reference_and_scattering": True,
                                "BFP_extraction_unit_guard_pass": bfp_unit_guard_pass,
                                "ROI_mapping_reversal": roi_mapping_reversal,
    ...
    detector_blank_guard_rows = [
        {
            ...
            "connection_physical_validity": next(
                ...
                if row["connection_state_id"] == "ET2030_BNC_direct_to_LI5640_current_input"
            ),
        ...
        },
        {
            ...
            "analytic_gaussian_FP_per_min": blank["analytic_gaussian_FP_per_min"],
            "rice_or_rayleigh_magnitude_FP_per_min": blank["rice_or_rayleigh_magnitude_FP_per_min"],
            "zero_event_upper_bound": blank["zero_event_upper_bound"],
        },
        {
            "stage": "R4_representative_full_wave_validation",
        ...
        route_decision_rows.append(
            {
                ...
                "reclassify_count": labels.count("reclassify_requires_external_review"),
                "inconclusive_count": labels.count("inconclusive_requires_plan_revision"),
                ...
                "sign_preserved_fraction": float(
                    np.mean(
                        [
                            row["sign_preserved"] == "True" or row["sign_preserved"] is True
                            for row in rows
                        ]
                    )
                ),
    ...
    for route in routes:
        ...
        for particle in particles:
            particle_rows = [
                ...
                if row["route_id"] == route_id and row["particle_id"] == particle["particle_id"]
            ]
            values = np.asarray([float(row["surrogate_delta_log"]) for row in particle_rows])
    ...
    context_rows.extend(
        {
            **row,
            "context_route_discussion_only": row["route_role"] in {"optional_robustness_probe"},
            "eligible_for_route_promotion": False,
            "promotion_requires_external_review": True,
        }
        for row in route_decision_rows
        ...
        thermal_summary_rows.append(
            {
                ...
                "n_cases": len(rows),
                "risk_states": "|".join(
                    sorted({str(row["thermal_artifact_risk"]) for row in rows})
                ),
    ...
    all_event_prob_blocked = all(
        row["event_probability_claim_level"] == "absolute_blocked" for row in observable_rows
    )
    no_legacy_names = all(
    ...
    final_decision_counts = {
        label: sum(row["final_route_validation_decision"] == label for row in route_decision_rows)
...
def _sign_preserved(left: float, right: float) -> bool:
    ...
    return (left_sign == 0 and right_sign == 0) or (left_sign != 0 and left_sign == right_sign)
...
def run_revised_R4_rerun(
    ...
    *,
    ...
    def sign_preserved_raw(row: dict[str, str]) -> bool:
        if row["particle_id"] == "blank":
            return True
        return _sign(float(row["full_wave_cross_term_signed_W"])) == _sign(surrogate_cross(row))
    ...
    for case_row, obs_row in zip(raw_case_rows, raw_observable_rows, strict=True):
        ...
        revised_observable = {
            **obs_row,
            **common_updates,
            "raw_full_wave_cross_term_signed_W": obs_row["full_wave_cross_term_signed_W"],
            ...
            "abs_full_wave_cross_term_W": abs(float(obs_row["full_wave_cross_term_signed_W"])),
    ...
    for route_id in sorted({row["route_id"] for row in revised_observable_rows}):
        ...
        for band in ("blank_excluded", "near_zero_ambiguous", "reliable"):
            ...
            reliability_summary_rows.append(
                {
                    "route_id": route_id,
                    "route_role": _first_row(
                        route_rows,
                        f"R4 revised observable rows for route {route_id}",
                    )["route_role"],
                    "sign_reliability_band": band,
                    "n_rows": len(rows),
                    ...
                    "sign_preserved_after_global_flip_fraction": _fraction(
                        sum(
                            row["sign_preserved_after_global_flip"] is True for row in nonblank_rows
                        ),
                        len(nonblank_rows),
                    ),
                    "R5_plan_preparation_authorized": False,
    ...
    for route_id in sorted({row["route_id"] for row in revised_observable_rows}):
        ...
        route_first = _first_row(rows, f"R4 revised route rows for {route_id}")
        route_role = route_first["route_role"]
        ...
        refined_summary_rows.append(
            {
                ...
                "review_refined_sign_preserved_after_global_flip_fraction": (refined_fraction),
        ...
        route_decision_rows.append(
            {
                "route_id": route_id,
                "wavelength_nm": route_first["wavelength_nm"],
                "width_nm": route_first["width_nm"],
                "depth_nm": route_first["depth_nm"],
                "route_role": route_role,
                "route_role_locked": True,
                "context_route_promotion_authorized": False,
                "route_promotion_eligible": False,
                "main_660_redefinition_authorized": False,
                "selected_annulus_replaces_all_crossing_ranking": False,
                "nonblank_sign_preserved_after_global_flip_fraction": (nonblank_fraction),
    ...
    cost_row = {
        **cost,
        ...
        "under_R4_revised_rerun_review_cap": cost["under_R4_revised_rerun_review_cap"],
    ...
    write_csv_rows(output / "revised_full_wave_observable_summary.csv", revised_observable_rows)
    write_csv_rows(output / "cross_term_convention_resolution_summary.csv", convention_rows)
    write_csv_rows(
...
def run_R4_2_main660_nearwall_mesh_adjudication(
    ...
    *,
    external_authorization: str = ("PASS_TO_R4_2_MAIN660_NEARWALL_MESH_ADJUDICATION_ONLY"),
    write_root_manifest: bool = False,
) -> dict[str, Any]:
    if external_authorization != ("PASS_TO_R4_2_MAIN660_NEARWALL_MESH_ADJUDICATION_ONLY"):
    ...
    source_paths = {
        ...
        "source_main660_ambiguity_check_checksum": (
            DEFAULT_REVISED_R4_RERUN_DIR / "main_660_near_wall_coarse_sign_ambiguity_check.csv"
        ),
    ...
    source_checksums = {key: sha256_file(path) for key, path in source_paths.items()}
    ...
    r4_particle_rows = {str(row["particle_id"]): row for row in r4_plan["particle_panel"]}
    material_by_id = {str(row["particle_id"]): row for row in r4_plan["particle_material_contract"]}
            ...
            for interface_state in interface_states:
                for polarization_state in polarization_states:
                    for mesh_level in mesh_levels:
                        ...
                        if particle_id != "blank":
                            abs_by_route_particle.setdefault((route_id, particle_id), []).append(
                                abs(full_cross)
                            )
    ...
    for raw in raw_fine_rows:
        ...
        sign_preserved_after_global_flip = sign_agrees(canonical, surrogate_cross, particle_id)
        common = {
        ...
        case_row = {
            **common,
            ...
            "solver_backend_available": solver_status["solver_backend_available"],
            "solver_backend_name": solver_result["solver_backend_name"],
            "solver_backend_version_recorded": solver_result["solver_backend_version"],
            "solver_execution_mode": solver_status["solver_execution_mode"],
            "solver_case_completed": solver_result["solver_case_completed"],
            "solver_call_path": solver_result["solver_call_path"],
            "solver_output_checksum": solver_result["solver_output_checksum"],
        ...
        observable_row = {
            **common,
            ...
            "sign_ambiguous_due_to_near_zero": (reliability_band == "near_zero_ambiguous"),
            ...
            "route_promotion_eligible": False,
            **{field: raw[field] for field in R4_2_ADJUDICATION_REQUIRED_MODE_LOBE_FIELDS},
            **provenance,
        }
        validate_required_output_fields(case_row)
        validate_required_output_fields(observable_row)
    ...
    for route in route_panel:
        ...
        route_fine = [
            ...
            if row["route_id"] == route_id and row["particle_id"] != "blank"
        ]
        route_fine_reliable = [row for row in route_fine if row["sign_reliable"] is True]
        route_review = [row for row in review_source_rows if row["route_id"] == route_id]
        route_coarse = [row for row in coarse_source_rows if row["route_id"] == route_id]
        ...
        fine_summary_rows.append(
            {
                ...
                "fine_confirm_sign_preserved_count": sum(
                    row["sign_preserved_after_global_flip"] is True for row in route_fine
                ),
    ...
    fine_summary_rows.append(
        {
            ...
            "fine_confirm_sign_preserved_count": sum(
                row["sign_preserved_after_global_flip"] is True for row in fine_nonblank
            ),
            ...
            "possible_future_gate_after_success": ("PASS_R4_2_RESULTS_PREPARE_R5_PLAN_ONLY"),
            "R5_plan_preparation_authorized": False,
            **_R4_2_provenance(
    ...
    write_csv_rows(output / "main660_fine_confirm_sign_summary.csv", fine_summary_rows)
    write_csv_rows(output / "mesh_level_role_adjudication_summary.csv", mesh_summary_rows)
    write_csv_rows(output / "BFP_lobe_resolved_cross_term_summary.csv", lobe_rows)
...
def _r5_required_float(row: dict[str, str], *names: str, row_index: int) -> float:
    ...
    if not math.isfinite(numeric):
        raise ValueError(
            f"R5 source row {row_index} has non-finite value {text!r} for columns {names!r}"
        )
    return numeric
...
def _r5_scenario_terms(
    ...
    thermal_404_log_multiplier = (
        -0.25 if wavelength_nm == 404 and scenario["scenario_role"] == "thermal_404_gate" else 0.0
    )
...
def run_R5_full_grid_v2(
    ...
    *,
    ...
    source_checksums = {
        ...
        "R4_2_run_manifest_checksum": run_manifest_provenance_checksum(
    ...
    with base_path.open(newline="", encoding="utf-8") as handle:
        for source_row_index, source_row in enumerate(csv.DictReader(handle), start=1):
            ...
            particle_diameter_nm = _r5_float(
                source_row, "particle_diameter_nm", default=float("nan")
            )
            ...
            for scenario in scenario_rows:
                ...
                case_manifest_rows.append(
                    {
                        **common,
                        "source_v1_summary_checksum": source_checksums["base_v1_summary_checksum"],
                        ...
                        "R5_plan_report_checksum": source_checksums["R5_plan_report_checksum"],
                    }
                )
                summary_row = {
                    **common,
                ...
                thermal_404_log_multipliers.append(float(terms["thermal_404_log_multiplier"]))
    ...
    unit_rows = [
        ...
        {
            "metric": "scenario_detector_SNR",
            "unit": "relative_proxy_absolute_blocked",
            "status": "pass",
        },
        {
            "metric": "expected_prior_score_proxy_count_at_10000",
            "unit": "proxy_count_not_observed_events",
            "status": "pass",
        },
    ]
    cost_row = {
...
def run_R5_1_route_role_stability_interpretation(
    ...
    *,
    ...
    source_checksums = {
        ...
        "R5_run_manifest_checksum": run_manifest_provenance_checksum(r5_dir / "run_manifest.json"),
    }
    plan_provenance = plan["provenance_freeze"]
    for field in R5_1_REQUIRED_PROVENANCE_FIELDS:
        if source_checksums[field] != plan_provenance[field]:
    ...
    cost_row = _first_row(
        _read_csv_dicts(r5_dir / "full_grid_v2_cost_estimate.csv"),
        "R5 full-grid cost estimate",
    )
    ...
    optional_mean = float(
        role_by_id["optional_robustness_probe"]["mean_detectability_relative_prior_score"]
    )
    ...
    top_context_route_id = str(
        _first_row(top_context_rows, "R5.1 top context route warnings")["route_id"]
    )
    ...
    claim_guard_rows = [
        ...
        },
        {
            ...
            "status": "pass"
            if detector["legacy_detector_SNR_output_header_emitted"] == "False"
            else "fail",
        },
        {
            "guardrail": "legacy_calibrated_detector_SNR_output_header_emitted",
            ...
            "status": "pass"
            if detector["legacy_calibrated_detector_SNR_output_header_emitted"] == "False"
            else "fail",
        },
        {
            "guardrail": "thermal_sidecar_used_to_increase_NODI_score",
            ...
            "status": "pass"
            if thermal["thermal_sidecar_used_to_increase_NODI_score"] == "False"
            else "fail",
        },
        {
    ...
    report = (
        ...
        f"- Top context warning route: {top_context_route_id}.\n"
...
def run_R5_2_bounded_scenario_prior_audit(
    ...
    *,
    ...
    source_checksums = {
        ...
        "R5_1_run_manifest_checksum": run_manifest_provenance_checksum(
    ...
    for scenario in scenarios:
        main_by_scenario[scenario] = _mean(
            [
                float(row["detectability_relative_prior_score"])
                for row in audit_rows
                if row["route_role"] == "main_660" and row["scenario_bundle"] == scenario
            ]
        )
        ...
        for scenario in scenarios:
            route_scenario_mean = _mean(
                [
                    float(row["detectability_relative_prior_score"])
                    for row in audit_rows
                    if row["route_id"] == route_id and row["scenario_bundle"] == scenario
                ]
            )
            if route_scenario_mean > main_by_scenario[scenario]:
                scenario_above += 1
        all_context_above_all_scenarios = all_context_above_all_scenarios and scenario_above == len(
            scenarios
        )
    ...
    manifest_rows = [
        {
            ...
            "context_routes_above_main_under_all_scenarios": (
                len(R5_2_ABOVE_MAIN_CONTEXT_ROUTE_IDS) if all_context_above_all_scenarios else 0
            ),
            "R6_plan_preparation_authorized": False,
            "R6_execution_authorized": False,
            "R5_followup_expansion_authorized": False,
    ...
    write_csv_rows(
        output / "R5_2_weak_reference_control_audit.csv",
        [
            {
...
def _r5_3_candidate_metrics(
    *,
    ...
    weak_values = [
        score(row) for row in audit_rows if row["route_role"] == "weak_reference_control"
    ]
    context_values = [
        score(row) for row in audit_rows if row["route_id"] in R5_2_ABOVE_MAIN_CONTEXT_ROUTE_IDS
    ]
    ...
    for route_id in R5_2_ABOVE_MAIN_CONTEXT_ROUTE_IDS:
        route_mean = _mean([score(row) for row in audit_rows if row["route_id"] == route_id])
    ...
    return {
        ...
        "weak_reference_delta_explained_fraction": explained_fraction(old_weak_delta, weak_delta),
        "context_family_delta_explained_fraction": explained_fraction(
            old_context_delta, context_delta
        ),
        "warning_resolved_by_candidate": (weak_mean <= main_mean and context_count_above_main == 0),
    }
...
def run_R5_3_route_prior_model_revision_audit(
    ...
    *,
    external_authorization: str = ("PASS_TO_BOUNDED_ROUTE_PRIOR_MODEL_REVISION_AUDIT_ONLY"),
    write_root_manifest: bool = False,
) -> dict[str, Any]:
    ...
    source_checksums = {
        "R5_2_manifest_checksum": sha256_file(r5_2_dir / "R5_2_scenario_prior_audit_manifest.csv"),
        ...
        "R5_2_decision_table_checksum": sha256_file(r5_2_dir / "R5_2_audit_decision_table.csv"),
        "R5_2_next_stage_matrix_checksum": sha256_file(
            r5_2_dir / "R5_2_next_stage_recommendation_matrix.csv"
        ),
        "R5_2_run_manifest_checksum": run_manifest_provenance_checksum(
    ...
    all_mean = _mean([float(row["detectability_relative_prior_score"]) for row in audit_rows])
    main_values = [
        float(row["detectability_relative_prior_score"])
        for row in audit_rows
        if row["route_role"] == "main_660"
        ...
        candidate_rows.append(
            {
                ...
                **meta,
                ...
                "candidate_weak_reference_mean": metrics["candidate_weak_reference_mean"],
                "candidate_context_family_mean": metrics["candidate_context_family_mean"],
        ...
        decomposition_rows.append(
            {
                ...
                "near_wall_PEG_transport_prior_term": float(scenario["peg_survival_factor"]),
                "detector_blank_prior_term": float(terms["blank_noise_multiplier"]),
                "thermal_404_sidecar_exclusion_term": float(terms["thermal_404_multiplier"]),
    ...
    for context_row in _read_csv_dicts(r5_2_dir / "R5_2_context_route_above_main_audit.csv"):
        ...
        old_mean = _mean([float(row["detectability_relative_prior_score"]) for row in route_rows])
    ...
    for route_id in sorted(selected_or_404_ids):
        ...
        first = _first_row(
            route_rows,
            f"R5.2 selected-annulus/404 audit rows for route {route_id}",
        )
        selected_and_404_rows.append(
            {
                "route_id": route_id,
    ...
    weak_row = _first_row(weak_rows, "R5.3 weak-reference interpretation rows")
    decision_rows = [
        {
            "decision_subject": "weak_reference_control",
            "audit_finding": "selected family-level width/depth prior resolves weak-reference warning",
            "selected_candidate_prior_id": selected_candidate_id,
            "interpretation": weak_row["interpretation"],
            "recommended_next_class": selected_future_recommendation,
            "R6_plan_preparation_authorized": False,
            "route_promotion_authorized": False,
            "main_660_redefinition_authorized": False,
    ...
    manifest_rows = [
        {
            ...
            "selected_candidate_dof_count": candidate_metadata[selected_candidate_id]["dof_count"],
...
def run_R6_route_prior_sensitivity_audit(
    ...
    *,
    external_authorization: str = ("PASS_TO_BOUNDED_R6_ROUTE_PRIOR_SENSITIVITY_AUDIT_ONLY"),
    write_root_manifest: bool = False,
) -> dict[str, Any]:
    ...
    source_checksums = {
        "R5_3_manifest_checksum": sha256_file(r5_3_dir / "R5_3_route_prior_revision_manifest.csv"),
        ...
        "R5_3_run_manifest_checksum": run_manifest_provenance_checksum(
    ...
    candidate_ids = [
        str(candidate_id)
        for candidate_id in plan["candidate_prior_sensitivity_design"]["candidate_prior_ids"]
    ]
    ...
    old_score_by_row_id = {
        row["R5_case_id"]: float(row["detectability_relative_prior_score"]) for row in audit_rows
    }
    main_old_mean = _mean(
        [
            float(row["detectability_relative_prior_score"])
        ...
        candidate_main_by_particle[candidate_id] = {
            particle: _mean(
                [
                    ...
                    * _r6_candidate_factor(row, candidate_id)
                    for row in audit_rows
                    if row["route_role"] == "main_660" and row["particle_name"] == particle
                ]
            )
            ...
            for route_id in R5_2_ABOVE_MAIN_CONTEXT_ROUTE_IDS | {"660_700x1500"}:
                route_mean = _mean(
                    [
                        ...
                        * _r6_candidate_factor(row, candidate_id)
                        for row in audit_rows
                        if row["route_id"] == route_id and row["particle_name"] == particle
                    ]
                )
            ...
            sensitivity_rows.append(
                {
                    ...
                    "context_vs_candidate_adjusted_main_delta": (candidate_score - main_candidate),
                    "context_vs_unadjusted_main_delta": candidate_score - main_old_mean,
                    "optional_900_vs_candidate_adjusted_main_delta": (optional_candidate_delta),
                    ...
                    "scenario_residual_above_main_count": scenario_counts[row["scenario_bundle"]],
    ...
    for candidate_id in candidate_ids:
        ...
        candidate_registry_rows.append(
            {
                **metadata,
                ...
                "context_scenario_rows_above_main_after_candidate": (context_scenario_above_count),
                "uses_route_specific_multiplier": False,
                "uses_scenario_specific_per_route_fit": False,
                "uses_particle_specific_empirical_fit": False,
                "changes_main_660_definition": False,
        ...
        for route_id in sorted({row["route_id"] for row in audit_rows}):
            ...
            first = _first_row(
                route_rows,
                f"R6 route-prior audit rows for route {route_id}",
            )
            ...
            route_factor_rows.append(
                {
                    ...
                    "width_nm": first["width_nm"],
                    "depth_nm": first["depth_nm"],
                    "mean_route_prior_factor": _mean(route_factor_accum[(candidate_id, route_id)]),
                    ...
                    "candidate_adjusted_delta_vs_main": (candidate_route_mean - main_candidate),
            ...
            route_family_rows.append(
                {
                    ...
                    "residual_delta_vs_candidate_adjusted_main": group_mean - main_candidate,
            ...
            main_rows.append(
                {
                    ...
                    "main660_score_retention_fraction": (candidate_route_mean / old_route_mean),
                    "main660_retention_warning": (candidate_route_mean / old_route_mean) < 0.85,
                    "main_660_route_role_locked": True,
                    "main_660_redefinition_authorized": False,
                    "route_promotion_authorized": False,
                    "claim_level": "relative_with_priors",
    ...
    selected_future_recommendation = (
        ...
        and int(selected_candidate["weak_reference_scenario_rows_above_main_after_candidate"]) == 0
        else "prepare_route_governance_revision_plan_for_external_review_only"
    )
    audit_decision = (
        "low_dimensional_width_prior_sensitivity_stable_prepare_next_stage_plan_only"
        if selected_future_recommendation == "prepare_next_stage_plan_for_external_review_only"
    ...
    scenario_budget = {
        "external_authorization": external_authorization,
        "audit_execution_type": ("bounded_existing_R5_artifact_route_prior_sensitivity_audit_only"),
        "selected_candidate_prior_id": selected_candidate_id,
        "selected_future_recommendation_class": selected_future_recommendation,
        "R7_plan_preparation_authorized": False,
        "R7_execution_authorized": False,
...
def run_R7_route_prior_mechanistic_decomposition_audit(
    ...
    *,
    ...
    source_checksums = {
        "R6_manifest_checksum": sha256_file(r6_dir / "R6_route_prior_sensitivity_manifest.csv"),
        "R6_candidate_registry_checksum": sha256_file(r6_dir / "R6_candidate_prior_registry.csv"),
        ...
        "R6_run_manifest_checksum": run_manifest_provenance_checksum(r6_dir / "run_manifest.json"),
    }
    for field in R7_REQUIRED_PROVENANCE_FIELDS:
        if source_checksums[field] != plan["provenance_freeze"][field]:
    ...
    r6_manifest = _first_row(
        _read_csv_dicts(r6_dir / "R6_route_prior_sensitivity_manifest.csv"),
        "R6 route-prior sensitivity manifest",
    )
    ...
    if int(r6_manifest["derived_candidate_rows_evaluated"]) != (R6_DERIVED_CANDIDATE_ROW_CAP):
    ...
    accepted_candidate_ids = [
        str(candidate_id) for candidate_id in accepted_band["accepted_explanatory_candidate_ids"]
    ]
    if not all(
    ...
    sensitivity_rows = _read_csv_dicts(r6_dir / "R6_candidate_prior_sensitivity_matrix.csv")
    selected_candidate_id = "global_width_quadratic_regularization"
    selected_rows = [
    ...
    if len({row["particle_name"] for row in selected_rows}) != (R5_V1_SOURCE_PARTICLE_NAME_COUNT):
    ...
    existing_fields = set(_first_row(selected_rows, "R7 selected candidate rows"))
    ...
    for row in plan["mechanistic_decomposition_design"]["candidate_mechanistic_prior_registry"]:
        ...
        accepted_band_rows.append(
            {
                ...
                "main660_score_retention_fraction": r6_row["main660_score_retention_fraction"],
    ...
    width_quad_900_rows = [
        {
            ...
            "warning_resolved_by_candidate": width_900["warning_resolved_by_candidate"],
            "main660_score_retention_fraction": width_900["main660_score_retention_fraction"],
            ...
            "optional_900_vs_unadjusted_main_delta": optional_900_by_candidate["width_quad_900"][
    ...
    for row in mechanistic_rows:
        ...
        factor_schema_rows.append(
            {
                "candidate_mechanistic_prior_id": row["candidate_mechanistic_prior_id"],
                "candidate_prior_family": family,
                "factor_schema": schema,
                "existing_artifact_columns_available": row["existing_artifact_columns_available"],
    ...
    for row in residual_rows:
        ...
        family_residual_accum.setdefault(key, []).append(float(row["residual_delta_vs_main"]))
    ...
    comparison_groups = {
        "gold_anchor": [
            row for row in residual_rows if str(row["particle_size_stratum"]).startswith("gold")
        ],
        "EV_like": [
            row for row in residual_rows if not str(row["particle_size_stratum"]).startswith("gold")
        ],
    }
    comparison_rows = []
    ...
    for candidate_id in ["reference_band_penalty", "BFP_alignment_risk"]:
        ...
        non_width_rows.append(
            {
                "candidate_prior_id": candidate_id,
                "R6_warning_resolved_by_candidate": row["warning_resolved_by_candidate"],
    ...
    audit_decision = "mechanistic_width_prior_supported_prepare_artifact_gap_register_only"
    next_stage_rows = []
    ...
    return {
        ...
        "existing_R5_rows_interpreted": R7_ROUTE_PRIOR_SOURCE_ROW_CAP,
        "mechanistic_candidate_count": len(mechanistic_rows),
        "executable_existing_artifact_mechanistic_candidate_count": (executable_mechanistic_count),
        "physical_operator_artifact_gap_count": physical_artifact_gap_count,
        "particle_stratum_residual_warning_count": len(particle_top_rows),
        "selected_future_recommendation_class": selected_future_recommendation,
        "audit_decision": audit_decision,
...
def run_R7_1_operator_artifact_validation_protocol_generation(
    ...
    *,
    ...
    if self_authorization != ("PASS_TO_R7_1_OPERATOR_ARTIFACT_VALIDATION_PROTOCOL_GENERATION_ONLY"):
    ...
    source_checksums = {
        "R7_manifest_checksum": sha256_file(r7_dir / "R7_mechanistic_decomposition_manifest.csv"),
        ...
        "R7_factor_schema_checksum": sha256_file(r7_dir / "R7_mechanistic_prior_factor_schema.csv"),
        ...
        "R7_run_manifest_checksum": run_manifest_provenance_checksum(r7_dir / "run_manifest.json"),
    }
    for field in R7_1_REQUIRED_PROVENANCE_FIELDS:
        if source_checksums[field] != plan["provenance_freeze"][field]:
    ...
    r7_manifest = _first_row(
        _read_csv_dicts(r7_dir / "R7_mechanistic_decomposition_manifest.csv"),
        "R7 mechanistic decomposition manifest",
    )
    ...
    for module in modules:
        protocol_rows_by_file[protocol_by_output[module["module_id"]]] = _r7_1_protocol_rows(module)
    ...
    manifest.update(
        {
            ...
            "R7_1_selected_future_recommendation_class": (selected_future_recommendation),
            "R8_plan_preparation_authorized": False,
            "R8_execution_authorized": False,
            "new_experiment_authorized": False,
            "operator_artifact_acquisition_started": False,
    ...
    return {
        ...
        "required_artifact_field_count": artifact_field_count,
        "selected_future_recommendation_class": selected_future_recommendation,
        "protocol_decision": ("operator_artifact_requirements_defined_prepare_gap_register_only"),
        "R8_plan_preparation_authorized": False,
        "R8_execution_authorized": False,
        "new_experiment_authorized": False,
        "context_route_promotion_authorized": False,
...
def run_R7_2_operator_artifact_gap_register_generation(
    ...
    *,
    ...
    source_checksums = {
        ...
        "R7_1_stop_gate_checksum": sha256_file(r7_1_dir / "R7_1_stop_gate_summary.csv"),
        "R7_1_next_stage_matrix_checksum": sha256_file(
            r7_1_dir / "R7_1_next_stage_recommendation_matrix.csv"
        ),
        "R7_1_run_manifest_checksum": run_manifest_provenance_checksum(
    ...
    r7_1_manifest = _first_row(
        _read_csv_dicts(r7_1_dir / "R7_1_operator_artifact_validation_manifest.csv"),
        "R7.1 operator artifact validation manifest",
    )
    ...
    output_files = {
        "reference_operating_band_artifact": ("R7_2_reference_operating_band_gap_register.csv"),
        "BFP_slit_ROI_alignment_operator_artifact": (
            "R7_2_BFP_slit_ROI_alignment_gap_register.csv"
        ),
        "fabrication_metrology_margin_artifact": (
        ...
        "wall_PEG_transport_proxy_artifact": ("R7_2_wall_PEG_transport_proxy_gap_register.csv"),
        "particle_stratum_residual_artifact": ("R7_2_particle_stratum_residual_gap_register.csv"),
        "optional_900_governance_diagnostic_artifact": (
            "R7_2_optional_900_governance_gap_register.csv"
        ),
    }
    ...
    for artifact in plan["artifact_gap_registry"]:
        ...
        if int(artifact["required_field_count"]) != len(protocol_rows):
            raise ValueError(f"R7.2 artifact field count mismatch: {artifact['artifact_id']}")
        ...
        gap_rows_by_file[output_files[artifact["artifact_id"]]] = _r7_2_artifact_gap_rows(
            artifact=artifact, protocol_rows=protocol_rows
        )
    ...
    manifest.update(
        {
            ...
            "R7_2_selected_future_recommendation_class": (selected_future_recommendation),
            "operator_artifact_acquisition_started": False,
            "bench_measurement_started": False,
            "experimental_validation_started": False,
            "R8_plan_preparation_authorized": False,
...
def run_v2_no_measured_data_closure(
    ...
    *,
    ...
    source_checksums = {
        "R7_2_manifest_checksum": sha256_file(r7_2_dir / "R7_2_operator_artifact_gap_manifest.csv"),
        ...
        "R7_2_stop_gate_checksum": sha256_file(r7_2_dir / "R7_2_stop_gate_summary.csv"),
        "R7_2_next_stage_matrix_checksum": sha256_file(
            r7_2_dir / "R7_2_next_stage_recommendation_matrix.csv"
        ),
        "R7_2_run_manifest_checksum": run_manifest_provenance_checksum(
        ...
        "v2_target_alignment_self_review_checksum": sha256_file(
            PROJECT_ROOT / "reports" / "85_EV_NODI_realism_v2_target_alignment_self_review.md"
        ),
    }
    for field in V2_CLOSURE_REQUIRED_PROVENANCE_FIELDS:
        if source_checksums[field] != plan["provenance_freeze"][field]:
    ...
    r7_2_manifest = _first_row(
        _read_csv_dicts(r7_2_dir / "R7_2_operator_artifact_gap_manifest.csv"),
        "R7.2 operator artifact gap manifest",
    )
    ...
    selected_future_recommendation = "hold_v2_closed_pending_separate_post_v2_validation_program"
...
def run_R4_route_model_revision_audit(
    ...
    *,
    ...
    validate_output_names(_first_row(observable_rows, "R4 route-model observable summary").keys())
    validate_output_names(_first_row(route_decision_rows, "R4 route-model decision table").keys())
    validate_output_names(_first_row(case_manifest_rows, "R4 route-model case manifest").keys())
    ...
    for convention in conventions:
        ...
        main_gate_met = bool(convention["allowed_for_recovery"]) and main_fraction >= main_threshold
        ...
        if bool(convention["allowed_for_recovery"]) and not main_gate_met and all_route_watch_met:
            ...
            bfp_rows.append(
                {
                    ...
                    "sign_preserved_fraction_all": _fraction(route_preserved, route_total),
    ...
    for route in route_decision_rows:
        ...
        route_bfp = _first_row(
            ...
            f"R4 route-model BFP sanity rows for route {route_id}",
        )
        sanity_rows.append(
            {
                "route_id": route_id,
    ...
    if bool(best_allowed["main_660_recovery_gate_met"]):
        ...
        final_audit_decision = "partial_convention_signal_but_main_660_recovery_gate_not_met"
    decision_table_rows = [
        {
    ...
    (output / "R4_route_model_revision_report.md").write_text(report, encoding="utf-8")
