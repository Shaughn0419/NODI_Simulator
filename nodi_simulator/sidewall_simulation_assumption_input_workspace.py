"""Create and audit simulation-assumption input CSV workspaces for sidewall routes."""
...
SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_VERSION = (
    "sidewall_simulation_assumption_input_workspace_v1"
)
SIDEWALL_SIMULATION_ASSUMPTION_INPUT_WORKSPACE_CLAIM_BOUNDARY = (
    "simulation_assumption_input_workspace_not_template_as_evidence"
)

TARGET_HEADER_ONLY_STATUS = "target_header_only_ready_no_simulation_assumption_rows"
TARGET_SIMULATION_ROWS_PRESENT_STATUS = (
    "target_simulation_assumption_rows_present_not_rewritten_by_workspace"
)
...
class SidewallSimulationAssumptionInputWorkspaceSpec:
    input_branch: str
...
@dataclass(frozen=True)
class SidewallSimulationAssumptionInputWorkspaceRow:
...
def build_simulation_assumption_input_workspace(
    specs: list[SidewallSimulationAssumptionInputWorkspaceSpec],
    *,
    create_missing_targets: bool,
) -> list[SidewallSimulationAssumptionInputWorkspaceRow]:
    return [
...
def _workspace_row(
    spec: SidewallSimulationAssumptionInputWorkspaceSpec,
    *,
    create_missing_targets: bool,
) -> SidewallSimulationAssumptionInputWorkspaceRow:
    ...
    return SidewallSimulationAssumptionInputWorkspaceRow(
        workspace_row_id=f"SIMULATION-ASSUMPTION-WORKSPACE-{spec.input_branch}",
        ...
        hard_fail_if="header_only_or_template_rows_counted_as_claim_inputs",
...
def _target_status(
    *,
    ...
    if target_data_rows:
        return TARGET_SIMULATION_ROWS_PRESENT_STATUS
...
# Backward-compatible aliases for artifacts produced before the route was
# renamed from "real evidence" to "simulation assumption".
...
TARGET_REAL_ROWS_PRESENT_STATUS = TARGET_SIMULATION_ROWS_PRESENT_STATUS
SidewallRealEvidenceInputWorkspaceSpec = SidewallSimulationAssumptionInputWorkspaceSpec
SidewallRealEvidenceInputWorkspaceRow = SidewallSimulationAssumptionInputWorkspaceRow
build_real_evidence_input_workspace = build_simulation_assumption_input_workspace
