"""
NODI Interferometric Simulator — Mie Engine
...
def _riccati_bessel(x: float, nmax: int):
    ...
    psi[0] = np.sin(x)
    psi[1] = np.sin(x) / x - np.cos(x) if x != 0 else 0.0

    chi[0] = np.cos(x)
    chi[1] = np.cos(x) / x + np.sin(x) if x != 0 else 0.0

    for n in range(1, nmax):
        psi[n + 1] = (2 * n + 1) / x * psi[n] - psi[n - 1] if x != 0 else 0.0
        chi[n + 1] = (2 * n + 1) / x * chi[n] - chi[n - 1] if x != 0 else 0.0
