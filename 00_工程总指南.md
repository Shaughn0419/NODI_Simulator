...
> 当前状态：2026-05-23 复核版。v1 full-grid、v2 无实测收口、post-v2 P0-P18 审计、P19/pre-3seed gate，以及口径 B 的 EV+gold exhaustive 3seed × 10000e shared-dual full-grid post-run analysis 均已完成当前一轮分析。当前读者入口是 `reports/140_exhaustive_ev_gold_fullgrid_3seed_10000e_postrun_analysis_20260523.md`；综合背景读 `reports/88_*`，v2 边界读 `reports/87_*` 和 `reports/84_*`。本文是工程总说明，不替代代码、测试或最终综合报告。
...
- 口径 B 当前状态：B1 用 Tsuyama Au/Ag anchors 做 reproduction-lens target fitting 并冻结 estimated parameter set；B2 最新 robust evidence 是 `reports/140` 审计的 3seed × 10000e shared-dual EV+gold full-grid。`fixed_660_gold` 与 `per_wavelength_gold` 是共享物理事件上的 normalization views，不是两次独立采样。EV 推荐只用 EV/exosome rows；gold rows 只作 anchor / Tsuyama diagnostics；488/532 只作 control/trend，最终推荐只在 404/660 内写。旧 `results/lens_b_ev_gold_fullgrid_1seed_20260513/` 是 2 ms legacy reference；B6/B7 1000e 是历史单 seed method overlay。
