...
> 当前状态：2026-06-12 no-data closure 版。当前读者入口是 `reports/140_exhaustive_ev_gold_fullgrid_3seed_10000e_postrun_analysis_20260523.md`；封板门、残余假设和审计 ledger 读 `reports/147_*` / `reports/148_*`；历史综合背景可读 `reports/88_*`。当前结论只允许写成 detector-surrogate candidate families：`404/W500` fixed-view candidate 与 `660/W800` per-wavelength-view candidate 并立；R1 与 C/D×V2 是 narrowed no-data gate 外的 deferred cells。本文是工程总说明，不替代代码、测试或最终综合报告。

## 一句话定位

这是一套面向设计比较的 NODI 干涉散射检测链模拟与 dashboard 系统。当前主链适合做可审计的 relative/proxy ranking、paper-aligned comparison 和 calibration-ready provenance；不能在没有实测标定或高保真闭环时宣称 absolute calibrated quantitative prediction、detector-resolved winner、真实 SNR/LOD/FPR 或生物学真值。
...
- 口径 B 当前状态：B1 用 Tsuyama Au/Ag anchors 做 reproduction-lens target fitting 并冻结 estimated parameter set；B2 最新 robust evidence 是 `reports/140` 审计的 3seed × 10000e shared-dual EV+gold full-grid，并由 `reports/147` / `reports/148` 收口。`fixed_660_gold` 与 `per_wavelength_gold` 是共享物理事件上的 normalization views，不是两次独立采样。EV 推荐只用 EV/exosome rows；gold rows 只作 anchor / Tsuyama diagnostics；488/532 只作 control/trend。当前最终写法是 `404/W500` fixed-view candidate 与 `660/W800` per-wavelength-view candidate 并立。旧 `results/lens_b_ev_gold_fullgrid_1seed_20260513/` 是 2 ms legacy reference；B6/B7 1000e 是历史单 seed method overlay。
...
- EV design recommendation 已拆成 relative/within-lambda 与 absolute/global green 两层；detector operator disagreement 会形成 caution 和 candidate-family boundary，不再直接硬杀 relative candidate ranking，也不得升级成 detector-resolved winner。
...
- Phase 2 Tsuyama paper-fit alignment 已作为 B1 target-fitting / frozen-parameter provenance 保留；不能被读成 raw physical calibration。B2 EV full-grid 使用 frozen B lens 做 EV application，但 run manifest 里 `gamma / snr_scale / snr_response_exp` 在本 full-grid runner 中是 metadata-only；runtime ranking 主要来自 D2.1 operator、2 ms lock-in、selected-annulus 和 readout fields。Phase 2 / B1 不改 global EV material defaults，也不覆盖 all-crossing engineering main-library ranking。
...
- 最新 full-grid post-run 分析：[reports/140_exhaustive_ev_gold_fullgrid_3seed_10000e_postrun_analysis_20260523.md](./reports/140_exhaustive_ev_gold_fullgrid_3seed_10000e_postrun_analysis_20260523.md)
- 当前文档逐项状态审计：[reports/142_current_documentation_status_audit_20260523.md](./reports/142_current_documentation_status_audit_20260523.md)
