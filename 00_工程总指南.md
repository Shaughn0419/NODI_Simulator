...
> 当前状态：2026-05-14 复核版。v1 full-grid、v2 无实测收口、post-v2 P0-P18 审计，以及口径 B 的 EV+gold full-grid application 均已完成当前一轮分析。当前读者入口是 `reports/88_*` v5.2.6，v2 边界入口是 `reports/87_*` 和 `reports/84_*`。本文是工程总说明，不替代代码、测试或最终综合报告。
...
- 口径 B 当前状态：B1 用 Tsuyama Au/Ag anchors 做 reproduction-lens target fitting 并冻结 estimated parameter set；B2 已在 `results/lens_b_ev_gold_fullgrid_1seed_20260513/` 完成 EV+gold full-grid（`32,032` rows，`seed=42`，`10000 events/case`）。EV 推荐只用 EV/exosome rows；gold rows 只作 anchor / Tsuyama diagnostics；488/532 只作 control/trend，最终推荐只在 404/660 内写。
