...
def test_real_current_scores_activate_unique_winner_jrc() -> None:
    rows, guards = build_winner_jrc_policy_review(
        route_formula_policy_rows=[
    ...
    assert by_route["ROUTE-CAND-001"].winner_current is True
    assert by_route["ROUTE-CAND-001"].JRC_current is True
    ...
    assert winner_guard.activation_allowed_now is True
