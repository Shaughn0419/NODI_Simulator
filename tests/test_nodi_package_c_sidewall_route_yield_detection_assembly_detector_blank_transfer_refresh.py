...
from tools.audits import (
    build_nodi_package_c_sidewall_route_yield_detection_assembly_detector_blank_transfer_refresh as builder,
)


def test_assembly_detector_blank_transfer_refresh_packet_builds() -> None:
    ...
    assert summary["transfer_intake_waiting_branch_rows"] == 2
...
def test_assembly_rows_preserve_routes_and_transfer_intake_statuses() -> None:
...
def test_branch_rows_wait_for_detector_blank_transfer_evidence() -> None:
    ...
    assert {row["implementation_can_start"] for row in transfer_rows} == {True}
    for row in rows:
...
def test_assembly_detector_blank_transfer_refresh_outputs_manifest(tmp_path: Path) -> None:
    payload = builder.build_payload()
