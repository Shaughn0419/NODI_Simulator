...
def test_wet_surface_observation_intake_packet_builds() -> None:
    ...
    assert summary["accepted_observation_rows"] == 0
    assert summary["no_observation_matrix_rows"] == 2
...
def test_wet_surface_observation_intake_rows_are_missing_not_claims() -> None:
    ...
    for row in rows:
        assert row["observation_validation_status"] == "wet_observation_missing_contract_ready"
        assert row["accepted_observation_current"] == "false"
...
def test_wet_surface_route_matrix_is_no_observation() -> None:
    ...
    for row in rows:
        ...
        assert row["accepted_endpoint_count"] == "0"
        assert row["missing_endpoint_count"] == "7"
        assert row["yield_current"] == "false"
