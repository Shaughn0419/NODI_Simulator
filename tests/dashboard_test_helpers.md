...
## 文件定位
- 类型：测试模块
- 模块摘要：未提供模块级 docstring。
- 当前职责：Dashboard 测试辅助模块，集中提供 AppTest 断言与构造工具。

## 主要符号
- 顶层函数：`assert_precompute_engineering_dataframe_columns`、`mock_dashboard_engineering_result`、`mock_dashboard_breakdown_case`、`build_freeze_probe_fixture_cases`、`assert_freeze_probe_report_contract`、`build_result_health_fixture_frame`、`assert_result_health_report_contract`
