...
from nodi_simulator.post_v2_sixth_bounded_solver_lane_execution import (
    P16_SOLVER_OUTPUT_CSV,
    build_artifact_manifest,
    build_p15_authorization_binding_manifest,
    build_solver_output_manifest,
    build_solver_rows,
    validate_artifact_manifest,
    validate_execution_registry,
    validate_p15_authorization_binding_manifest,
...
REGISTRY_PATH = "configs/realism_v2/sixth_bounded_solver_lane_execution_registry.yaml"
PLAN_PATH = "reports/117_EV_NODI_P16_sixth_bounded_solver_lane_execution_plan.md"
README_PATH = "results/post_v2_sixth_bounded_solver_lane_execution/README.md"
P15_BINDING_PATH = (
    "results/post_v2_sixth_bounded_solver_lane_execution/"
    "sixth_bounded_solver_lane_execution_p15_authorization_binding_manifest.json"
)
SOLVER_OUTPUT_MANIFEST_PATH = (
    "results/post_v2_sixth_bounded_solver_lane_execution/"
    "sixth_bounded_solver_lane_trace_output_manifest.json"
)
ARTIFACT_MANIFEST_PATH = (
    "results/post_v2_sixth_bounded_solver_lane_execution/"
    "sixth_bounded_solver_lane_execution_artifact_manifest.json"
)
SCHEMA_DOCS = (
...
def _registry() -> dict:
    return rv2.load_json_yaml("sixth_bounded_solver_lane_execution_registry.yaml")
...
def _csv_rows() -> list[dict[str, str]]:
    with root_path(P16_SOLVER_OUTPUT_CSV.as_posix()).open(newline="", encoding="utf-8") as handle:
        return list(csv.DictReader(handle))


def test_p16_registry_authorizes_only_sixth_bounded_trace_execution() -> None:
    registry = validate_execution_registry(_registry())

    assert registry["stage"] == "P16_sixth_bounded_solver_lane_execution_complete"
    assert registry["execution_role"] == "sixth_bounded_solver_lane_execution_trace_only"
    assert registry["physical_solver_execution_authorized"] is True
    assert registry["sixth_bounded_solver_lane_execution_authorized"] is True
...
def test_p16_generated_artifacts_are_current() -> None:
    assert _load_json(P15_BINDING_PATH) == build_p15_authorization_binding_manifest(root_path("."))
...
def test_p16_sixth_lane_output_is_rank_pairwise_trace_only() -> None:
    rows = validate_solver_rows(build_solver_rows(root_path(".")))
    csv_rows = _csv_rows()

    assert len(rows) == 3
    assert [row["sixth_lane_response_rank"] for row in rows] == [1, 2, 3]
    assert [row["sixth_lane_pairwise_order_signature"] for row in rows] == [
        "1_of_3",
        "2_of_3",
        "3_of_3",
    ]
    assert [row["sixth_lane_vs_p14_rank_delta"] for row in rows] == [-1, 1, 0]
    ...
    assert "solver_native_sixth_lane_response_trace_only" in csv_rows[0]


def test_p16_binding_and_output_manifests_preserve_boundaries() -> None:
    binding = validate_p15_authorization_binding_manifest(
        build_p15_authorization_binding_manifest(root_path("."))
    )
    ...
    for manifest in (binding, output, artifact):
        ...
        assert manifest["sixth_bounded_solver_lane_execution_authorized"] is True
...
def test_p16_records_required_phrase_and_binds_p15_gate() -> None:
    binding = validate_p15_authorization_binding_manifest(
        build_p15_authorization_binding_manifest(root_path("."))
    )

    assert binding["required_future_authorization_phrase"] == P15_REQUIRED_FUTURE_AUTHORIZATION_PHRASE
    assert binding["user_authorization_phrase_received"] == P15_REQUIRED_FUTURE_AUTHORIZATION_PHRASE
    assert binding["p15_gate_prior_decision"] == "not_authorized_pending_explicit_future_request"
    assert binding["bound_route_count"] == 3


def test_p16_registry_and_manifests_reject_scope_tampering() -> None:
    ...
    binding = build_p15_authorization_binding_manifest(root_path("."))
    binding["p14_trace_context_path"] = "results/other.csv"
    with pytest.raises(ValueError, match="P14 trace context path drifted"):
        validate_p15_authorization_binding_manifest(binding)
...
def test_p16_text_artifacts_use_blocker_language() -> None:
...
def test_p16_report_and_readme_are_in_claim_scan() -> None:
...
def test_p16_registry_is_excluded_from_p0_review_package_config_glob() -> None:
    paths = {entry["path"] for entry in _config_entries(root_path("."))}

    assert REGISTRY_PATH not in paths


def test_p16_verifier_cli_passes() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(root_path("tools/verify_post_v2_sixth_bounded_solver_lane_execution.py")),
            "--package-root",
            str(root_path(".")),
        ],
        check=True,
    ...
    assert "PASS sixth_bounded_solver_lane_execution_registry" in result.stdout
    assert "PASS sixth_bounded_solver_lane_execution_output_csv_current" in result.stdout
    assert "PASS sixth_bounded_solver_lane_execution_claim_boundaries" in result.stdout
