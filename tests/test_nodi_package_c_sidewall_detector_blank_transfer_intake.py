...
def test_detector_blank_transfer_intake_packet_builds_without_transfer_input() -> None:
    ...
    assert summary["accepted_transfer_rows"] == 0
    assert summary["no_transfer_matrix_rows"] == 2
...
def test_detector_blank_transfer_rows_bind_detector_panel_routes() -> None:
    ...
    assert {row["transfer_validation_status"] for row in intake_rows} == {
        "detector_blank_transfer_missing_panel_ready"
    }
    assert {row["route_transfer_matrix_status"] for row in matrix_rows} == {
        builder.ROUTE_MATRIX_NO_TRANSFER_STATUS
    }
