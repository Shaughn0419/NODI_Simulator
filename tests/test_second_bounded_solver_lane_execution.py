...
from nodi_simulator.post_v2_second_bounded_solver_lane_execution import (
    P8_SOLVER_OUTPUT_CSV,
    build_artifact_manifest,
    build_p7_authorization_binding_manifest,
    build_solver_output_manifest,
    build_solver_rows,
    validate_artifact_manifest,
    validate_execution_registry,
    validate_p7_authorization_binding_manifest,
...
REGISTRY_PATH = "configs/realism_v2/second_bounded_solver_lane_execution_registry.yaml"
PLAN_PATH = "reports/105_EV_NODI_P8_second_bounded_solver_lane_execution_plan.md"
README_PATH = "results/post_v2_second_bounded_solver_lane_execution/README.md"
P7_BINDING_MANIFEST_PATH = (
    "results/post_v2_second_bounded_solver_lane_execution/"
    "second_bounded_solver_lane_execution_p7_authorization_binding_manifest.json"
)
SOLVER_OUTPUT_MANIFEST_PATH = (
    "results/post_v2_second_bounded_solver_lane_execution/"
    "second_bounded_solver_lane_trace_output_manifest.json"
)
ARTIFACT_MANIFEST_PATH = (
    "results/post_v2_second_bounded_solver_lane_execution/"
    "second_bounded_solver_lane_execution_artifact_manifest.json"
)
SCHEMA_DOC_PATHS = (
...
def _registry() -> dict:
    return rv2.load_json_yaml("second_bounded_solver_lane_execution_registry.yaml")
...
def _csv_rows() -> list[dict[str, str]]:
    with root_path(P8_SOLVER_OUTPUT_CSV.as_posix()).open(newline="", encoding="utf-8") as handle:
        return list(csv.DictReader(handle))


def test_p8_registry_authorizes_only_second_bounded_trace_execution() -> None:
    registry = validate_execution_registry(_registry())

    assert registry["stage"] == "P8_second_bounded_solver_lane_execution_complete"
    assert registry["execution_role"] == "second_bounded_solver_lane_execution_trace_only"
    ...
    assert registry["second_bounded_solver_lane_execution_authorized"] is True
...
def test_p8_generated_artifacts_are_current() -> None:
    assert _load_json(P7_BINDING_MANIFEST_PATH) == build_p7_authorization_binding_manifest(
        root_path(".")
    )
...
def test_p8_second_lane_output_is_rank_pairwise_trace_only() -> None:
    rows = validate_solver_rows(build_solver_rows(root_path(".")))
    csv_rows = _csv_rows()

    assert len(rows) == 3
    assert [row["second_lane_response_rank"] for row in rows] == [1, 2, 3]
    assert [row["second_lane_pairwise_order_signature"] for row in rows] == [
        "1_of_3",
        "2_of_3",
        "3_of_3",
    ]
    assert [row["second_lane_vs_p6_rank_delta"] for row in rows] == [0, 0, 0]
    ...
    assert "solver_native_second_lane_response_trace_only" in csv_rows[0]


def test_p8_binding_and_output_manifests_preserve_boundaries() -> None:
    binding = validate_p7_authorization_binding_manifest(
        build_p7_authorization_binding_manifest(root_path("."))
    )
    ...
    for manifest in (binding, output, artifact):
        ...
        assert manifest["second_bounded_solver_lane_execution_authorized"] is True
...
def test_p8_records_required_phrase_and_binds_p7_gate() -> None:
    binding = validate_p7_authorization_binding_manifest(
        build_p7_authorization_binding_manifest(root_path("."))
    )

    assert binding["required_future_authorization_phrase"] == P7_REQUIRED_FUTURE_AUTHORIZATION_PHRASE
    assert binding["user_authorization_phrase_received"] == P7_REQUIRED_FUTURE_AUTHORIZATION_PHRASE
    assert binding["p7_gate_prior_decision"] == "not_authorized_pending_explicit_future_request"
    assert binding["bound_route_count"] == 3


def test_p8_registry_and_manifests_reject_scope_tampering() -> None:
    ...
    binding = build_p7_authorization_binding_manifest(root_path("."))
    binding["route_subset_binding_path"] = "results/other.json"
    with pytest.raises(ValueError, match="route subset binding path drifted"):
        validate_p7_authorization_binding_manifest(binding)
...
def test_p8_text_artifacts_use_blocker_language() -> None:
...
def test_p8_report_and_readme_are_in_claim_scan() -> None:
...
def test_p8_registry_is_excluded_from_p0_review_package_config_glob() -> None:
    paths = {entry["path"] for entry in _config_entries(root_path("."))}

    assert REGISTRY_PATH not in paths


def test_p8_verifier_cli_passes() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(root_path("tools/verify_post_v2_second_bounded_solver_lane_execution.py")),
            "--package-root",
            str(root_path(".")),
        ],
        check=True,
    ...
    assert "PASS second_bounded_solver_lane_execution_registry" in result.stdout
    assert "PASS second_bounded_solver_lane_execution_output_csv_current" in result.stdout
    assert "PASS second_bounded_solver_lane_execution_claim_boundaries" in result.stdout
