# EV/NODI 综合分析报告 v5.1：reader table supplement on top of v5.0 reader-centric restructure

- 日期：2026-05-12（v5.1 reader table 增强）；v5.0 主体重构日期：2026-05-11
- 版本：**v5.1（reader table supplement）**——在 v5.0 reader-centric 重构基础上做"二次精修"+ "读者表格 / 解释增强"。新增 §0.3b 读者问题导航表 + §3.0a 阶段量级联解释 + §6.4.F + §7.0 变量固定法 + §7.2.5a 信号链扩列对比 + §8.5 blocker 分类 + §8.6 noise sensitivity + §10.6 多候选裁决表 + §11.8 step 流程表 + §13.2 阶段量来源映射；扩展 §2.4 到 4 波长。
- **所有数值结论、forbidden claim、release status 和冻结参数与 v5.0 / v4.2 完全一致**；本版只补读者表格 / 增解释，不引入新计算、新候选、新 lane、新随机种子、新 solver case、新 experiment、新 measured artifact ingest。
- 报告性质：读者向综合分析报告
...
> **倍数计算口径**：本表所有"相对 660 nm"列都是 `量(λ) / 量(660 nm)` 的比值，baseline 是同一 W × H 同一 EV biomimetic panel 下的 660 nm。常数倍率（例如干涉项里的 `2`）在分子分母同时出现时已抵消，所以**不会再乘进相对倍数列**。`transit (ms)` 用绝对值；`detection proxy (%)` 用绝对百分比。
...
| EV biomimetic synthetic detection proxy（口径 A 同 lens；§6.4 表 6.4.A 路线类 mean）| 12.61% (main) | ≈ 7.77% (mid-wave class avg, 同 lens 但混合几何)| ≈ 7.77% | 4.45% (shortwave probe class) | mid-wave class 的 488 / 532 没有按 λ 拆分；shortwave class 对应 404 / 600 × 1300 |
...
**一句话 takeaway（表 8.6）**：detection 低**从来不是单一黑箱失败**——v5.0 内每条低 detection / 未晋升路线都能映射到上面 10 类 blocker 之一（或多条叠加）。所以"为什么检测不到"必须先指明 blocker 类别，再讨论用什么 measured artifact（§16.2）能解锁哪一类。
...
| 假想 reference×2（参考场翻倍） | ≈ 2× peak（cross term 翻倍）| 2.00× | 1.00× | margin z ≈ 1.5–2× | 估计中等改善，受 threshold 上抬抵消 | reference↑ 让 amplitude SNR↑，但 detection 还要过 threshold / margin 关 |
...
比较 weak-ref 行 与假想 reference×2：
  reference 的方向性影响是双向的——太弱 (cross term↓ → peak↓)，太强 (amplitude SNR↑ 但 threshold 同步↑)。
  最优 reference 不是"越大越好"，而是 cross term 与 noise / threshold 平衡的 operating band。
  这是 §10.4 width-prior 模型 (W/800)^1.5/^2 之所以"补缺"而不是"打压"的物理直觉支持。
```

**一句话 takeaway（表 8.7）**：reference 增强 → amplitude SNR ↑，但 detection 还要过 threshold / margin / transit / gate 四关。所以"reference 放大了 N 倍噪声却还重要"的根本答案是：**noise 不是被参考场放大或抑制，它进入的是另一个完全独立的环节（threshold + margin），这条独立路径既不被参考场放大也不被参考场抑制**。
...
| candidate | peak signal（相对 main） | synthetic detection score (%) | transit (ms) | noise / margin 风险 | paper-likeness | route governance | why yes / why not |
|---|---|---:|---:|---|---|---|---|
| **660 / 800 × 1400 (main 1)** | 1.00× baseline | 12.52 (all-cross) / 45.15 (NODI lens) | 8.94 | 中等（baseline）| 与 2022 NODI 几何同方向 | **conditional_relative_main**；近壁细网格通过 1.0；P6 / P8 / P14 trace 首位 | **WHY YES**：通过所有 5 条主裁决：物理稳定 + width-prior 可接受 + 近壁细网格 + R5.2 sidecar + P0 audit + paper trend 对齐 |
| **660 / 800 × 1500 (main 2)** | ≈ 1.00× | 12.70 (all-cross) | 8.94 | 中等 | 同 main 1 | **conditional_relative_main**；近壁细网格通过 1.0；P10 / P12 / P16 trace 首位 | **WHY YES**：与 main 1 在 trace 间互换首位，作为集合保留；不挑单一冠军 |
| 660 / 500 × 1500 | 略低（参考场偏离 baseline）| **19.64**（数字最高）| 8.94 | 高（窄通道工程风险被原模型低估）| 偏离论文器件 | **context, surrogate_sensitive_not_promoted**（ratio_vs_main 1.557）| **WHY NOT**：§10.4 width-prior `(W/800)^1.5/^2` 加补缺后越线消失；R5.2 sidecar `context_route_promotion_authorized = false`；P0 audit 把 563 条类似路线降级 |
| 660 / 700 × 1500 | 偏低（\|A_ref\| 弱）| 15.23 | 8.94 | 中等-高（reference too weak）| 偏离论文器件 | **weak_reference_control_only**；R5.2 显示 weak-ref 8 / 8 scenarios above main | **WHY NOT**：detection 看似高但 \|A_ref\| 偏低 → cross term 弱 → 不是物理优势；治理上仅作 control |
| 660 / 900 × 1400 | ≈ 1.00× | 12.30 | 8.94 | 中等 | 偏离论文器件 | **optional_robustness_probe_only** | **WHY NOT (as main)**：detection 与 main 持平但治理边界禁止 main-660 redefinition；保留为 probe |
...
| 660 / 800 × 550（Tsuyama 论文 depth）| ≈ 1.00× | 47.15 (NODI lens) / — (all-cross) | 8.94 | 中等 | **最接近论文器件几何** | **paper-sanity / cross-check**；不替代 main-660 | **WHY NOT (as main-660)**：paper-likeness 高但 v5.0 main-660 治理选择**不依赖**单纯"接近 Tsuyama 几何"；保留为 cross-check 几何 |
| 口径 B frozen diagnostic set: 660 / 1200 × 550 + 660 / 800 × 550（Au panel + selected-annulus）| — | — (口径 B 用 reproduction score, **不**用 detection 排名)| 8.94 | — | step 1 校准 lens 内 raw Au peak-height exponent residual 最低 (3.0335 / 3.0456) | **conditional diagnostic set, release_status = negative_or_diagnostic_result_only** | **WHY YES (as 口径 B)**：在 step 1 校准过的 reproduction lens 内 residual 最低（**不是因为最像 Tsuyama 几何**；§11.4 关键澄清）|
...
> 2. **风险先验补缺**：window-prior `(W/800)^1.5/^2` 加补缺后不被压损（main 保留比 ≥ 0.9）
...
| **v5.1** | **2026-05-12** | **reader table supplement**（codex 第二轮反馈吸收）：新增 §0.3b 读者问题导航表 + §3.0a 阶段量级联解释 + 7.0 变量固定法 + §7.2.5a 信号链扩列对比（2 张代表性表）+ §8.5 blocker 分类表 + §8.6 噪声 sensitivity 数值表 + §10.6 多候选裁决表 + §11.8 口径 B step 流程表 + §13.2 阶段量来源映射表；扩展 §2.4 到 4 波长机制链。**不引入新计算**；数值与 forbidden 与 v5.0 完全一致。|
...
### 18.4 v5.1 reader table supplement 的 8 项验收（codex 第二轮反馈映射）

v5.1 仅做读者表格 / 解释增强，不改变 v5.0 / v4.2 任何数值或 forbidden。验收按 codex 8 项读者问题：
...
2. v5.1 不引入任何新 simulation / 新 case / 新 candidate / 新 lane / 新 random seed
   / 新 solver case / 新 measured artifact ingest；
3. v5.1 所有新表的数据都来自 §6 / §7 / §11.2 / §14 / §17 已发布行的派生 / 重组；
4. v5.1 所有新表都按 v5.0 二次精修后的术语口径写：
   ...
   - 倍数列只用于阶段量（Csca× / |E_sca|× / |A_ref|× / cross-term× / peak× / noise×），
     且每张表都明确 baseline；
5. 跨 lens / panel 的对比一律按 §7.0 变量固定法 5 条进行，
...
（v5.1 报告主体结束。v5.1 reader table supplement 验收映射见 §18.4；不变量见 §18.3 + §18.5；下一步硬依赖见 §16。）
