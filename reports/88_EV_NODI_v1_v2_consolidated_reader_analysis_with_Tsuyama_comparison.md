# EV/NODI 综合分析报告 v5.2：reader comparison table pack + v5.1.1 hotfix

- 日期：2026-05-12（v5.1.1 hotfix + v5.2 reader comparison pack）；v5.1 reader table 初版日期：2026-05-12；v5.0 主体重构日期：2026-05-11
- 版本：**v5.2（reader comparison table pack）**——在 v5.1 (4106d38) 基础上：
  ...
  - **v5.2 reader comparison pack**：新增附录 D（5 张固定 W × H vary λ 表）+ 附录 E（5 张固定 λ vary W × H 表）+ 附录 F（strict controlled vs route-level 14 行对照矩阵）。每张表都明确 source type / strict controlled? / baseline / lens / panel，detection cell 只在 v5.x 有 strict 直接行时填数字，否则填 `—` 并在 note 列说明最近邻。
- **所有数值结论、forbidden claim、release status 和冻结参数与 v5.0 / v4.2 完全一致**。**精确表述**：v5.1 / v5.1.1 不引入新 simulation / solver case / candidate / lane / random seed / measured artifact ingest；v5.1 新增内容仅为既有结果的**派生重组、单位换算、物理近似解释和读者向诊断表**（包括 §6.4.F 信号链汇总、§7.2.5a 信号链扩列、§8.6 假想 sensitivity 数值版、§13.2 阶段量来源映射等"derived reader table"）。这些派生表的 baseline / source / aggregation method 都在表头或下方明确标注。
- 报告性质：读者向综合分析报告
...
| **v5.2** | **2026-05-12** | **reader comparison table pack**：附录 D（5 张固定 W × H vary λ derived reader tables）+ 附录 E（5 张固定 λ vary W × H derived reader tables）+ 附录 F（strict controlled vs route-level 14 行对照矩阵——v5.2 防误读核心）。每张表明确 source type / strict controlled / baseline；detection cell 只在 strict 直接行时填，否则 `—` + note 最近邻。**不引入新计算**。|
...
7. v5.2 附录 D / E 中 detection cell **只在 v5.x 有 strict 直接行时填数字**；
...
codex 第三轮反馈认为旧 report 47 派生 CSV (`mechanism_chain_by_wavelength_EV_medians.csv` 等) 存在于本地。**v5.1.1 验证后修正**：以下文件**当前 checkout 内不存在**：

```text
results/ev_nodi_realism_v2_full_grid_R5_v2/mechanism_chain_by_wavelength_EV_medians.csv
results/ev_nodi_realism_v2_full_grid_R5_v2/paired_wavelength_effect_ratios_EV.csv
results/ev_nodi_realism_v2_full_grid_R5_v2/geometry_effect_ranges_EV.csv
results/ev_nodi_realism_v2_full_grid_R5_v2/within_lambda_top_geometries.csv
results/ev_nodi_realism_v2_full_grid_R5_v2/wavelength_404_to_660_signal_decomposition.csv
results/ev_nodi_realism_v2_full_grid_R5_v2/reference_operating_point_by_lambda.csv
```

通过 `find /Volumes/Storage-Mac/nodi_simulator（usb）/results /Volumes/Storage-Mac/nodi_simulator（usb）/archive` 验证未发现。这意味着这些表要么在另一台机器上存在，要么是 codex 误以为存在（可能基于历史 commit 中的引用）。

**v5.2 决定**：迁移延后到这些 artifact 实际可用时；附录 D / E 的覆盖已经满足 codex 验收清单 8 项 / 10 项的核心需求（剩余依赖已在 §16.2 P19 evidence-strategy gate 实测优先级中明示）。
...
| 660 | **strict 直接行** | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 8.94 | **12.52** (all-cross) / **45.15** (NODI lens) | main, no blocker | §6.4 表 6.4.A `main_660_W800_D1400` 直接行 |

> **takeaway D.1**：660 nm 是此几何下唯一有 strict 直接行的波长；其他 3 个波长只能从阶段量推导方向（短波 peak↑ + transit↓），detection cell 不可填。
...
| 532 | strict 物理 + 口径 B note | 2.37 | 1.54 | ≈ 1.00 | ≈ 1.54 | 2.37 | ≈ 1.2–1.4 | 1.0 | 7.21 | — (EV) ; 口径 B raw Au exponent = 3.1563 (532 / 800 × 550 D2.1 best) | EV 口径无 strict；口径 B 仅给 raw Au exponent | §11.2.3 case-level decomposition |
| 660 | **strict 直接行** | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 8.94 | **47.15** (NODI lens) / **43.50** (2020 paper lens) / **46.70** (2022 NODI paper lens) | paper-sanity / cross-check | §6.4 表 6.4.B 直接行；3 个 lens 都直接覆盖 |
...
| W × H (nm) | source type | Csca status | \|A_ref\|× | cross-term× | peak× | noise× | transit (ms) | synthetic detection score (%) | route role | blocker |
|---|---|---|---:|---:|---:|---:|---:|---:|---|---|
| 800 × 550 | strict 直接行（NODI lens / 2020 paper lens / 2022 NODI paper lens）| constant for same particle panel | ≈ 1.0 (≈ baseline 几何)| 1.0 | 1.0 | 1.0 | 8.94 | 47.15 (NODI) / 43.50 (2020) / 46.70 (2022)；**all-cross 此 H 未直接覆盖** | paper-sanity / cross-check | — (跨 lens 不可与 800 × 1400 比) |
| 800 × 1200 | qualitative only | 同 | 接近 baseline | ≈ 1.0 | ≈ 1.0 | 1.0 | 8.94 | — | 此 H 在 W=800 v5.x 未直接覆盖 | — |
| **800 × 1400 (main)** | strict 直接行 | 同 | 1.0 baseline | 1.0 | 1.0 | 1.0 | 8.94 | **12.52** (all-cross) / **45.15** (NODI lens) | **conditional_relative_main** | — |
| **800 × 1500 (main)** | strict 直接行 | 同 | ≈ 1.0 | 1.0 | 1.0 | 1.0 | 8.94 | **12.70** (all-cross)；NODI lens 此 H 未覆盖 | **conditional_relative_main** | — |
...
| W × H (nm) | source type | Csca status | \|A_ref\|× | cross-term× | peak× | noise× | transit (ms) | synthetic detection score (%) | route role | blocker |
|---|---|---|---:|---:|---:|---:|---:|---:|---|---|
| 500 × 800 | strict 直接行 (NODI lens / 2020 paper lens / 2022 NODI paper lens) | constant for same particle panel | ≈ 0.97 (浅通道) | ≈ 2.6 (相对 660 同 W × H baseline) | ≈ 2.0 | 1.0 | 5.48 | 33.65 (NODI) / 34.50 (2020) / 35.40 (2022) | 短波 lens cross-check | transit / gating mismatch |
...
| §6.4 表 6.4.F 4 波长信号链汇总 (800×1400 同 W×H)| ⚠️ 阶段量 strict / detection route-level | W × H + panel | λ | 阶段量 ✅ ；detection ❌ | 阶段量方向；detection 仅 660 cell 是 strict 直接行 |
...
| 附录 D.1–D.5 各表 | 阶段量 ✅；detection 列只对 660 cell 有 strict 直接行 | W × H + panel | λ | 阶段量 ✅；detection 仅 strict 直接行 cell | 同 §6.4.F |
