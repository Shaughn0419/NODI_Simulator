...
### 2.4 强参考场极限下的具体放大表（404 vs 660 nm，EV biomimetic + 相近几何）

> **倍数计算口径**：本表所有"404 nm 相对值"列都是 `量(404 nm) / 量(660 nm)` 的比值。常数倍率（例如干涉项里的 `2`）在分子分母同时出现时已抵消，所以**不会再乘进相对倍数列**。

| 量 | 660 nm 基线 | 404 nm 相对值 | 物理 / 推导来源 |
|---|---:|---:|---|
| Mie 散射截面 Csca | 1.00× | 7.10× | EV Rayleigh: (660 / 404)⁴ = 7.10 |
| 收集场幅值 \|E_sca\| | 1.00× | 2.66× | √Csca = √7.10 |
| 参考场幅值 \|E_ref\| | 1.00× | ≈ 0.95–1.00× | §6.3 衍射对照：404 nm 系列 \|E_ref\| 在 0.91–0.97 范围（相对 660 / 800×550 = 1.000），660 nm 系列在 0.96–1.00；同 geometry 下 404 略低于 660，但波长方向因子不超过 ±10% |
| 干涉一次项幅值 \|E_ref\|·\|E_sca\| | 1.00× | ≈ 0.95 × 2.66 ≈ 2.5× | 因子 2 在比值中抵消（不再乘进），所以**不是** 5.5× |
| 实际 peak（含 cos Δφ 运行平均，约 0.7–0.9 因子）| 1.00× | ≈ 2.0× | 干涉项 2.5× × cos Δφ 平均 0.7–0.9 ≈ 1.7–2.3×；与用户示例表 2.01× 同 order |
| transit 时间窗（与几何无关，只看 λ）| 8.94 ms | 5.48 ms（≈ 0.61×）| beam waist ∝ λ / NA → 短波 transit 短 |
| 综合 detection | 1.00× | ≈ 0.4× | §6.4 表 6.4.B / §7.1：peak 翻倍但 transit↓、phase flip↑、margin↓ → detection 反而下降 |
