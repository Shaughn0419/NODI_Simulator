# P8 Second Bounded Solver Lane Execution Plan

P8 uses the user-provided authorization phrase `authorize second bounded solver lane execution` to run one narrow second bounded trace lane. It binds P7 authorization design artifacts, the P4 reviewed three-route input subset, and P6 trace context.

The P8 output is a deterministic bounded phase-gradient trace for the same three routes selected in P4/P6. Interpretation remains limited to rank, rank-percentile, pairwise order, and rank delta versus the P6 trace ordering. Solver-native trace fields are not final gates.

P8 does not authorize calibrated claims, measured or calibration data ingest, new mesh generation, operator export, full-wave solver execution, vector solver execution, roughness leakage simulation, transport residence-time simulation, route promotion, main-660 redefinition, or optional_660_W900_D1400 redefining main-660.
...
- P7 authorization gate record and candidate lane contract.
- P4 route subset input manifest with exactly three selected routes.
- P6 trace output used as prior trace context, not as calibration or promotion evidence.
