# NODI Package C Sidewall Route Accepted Evidence Smoke

Disposition: `NODI_PACKAGE_C_SIDEWALL_ROUTE_ACCEPTED_EVIDENCE_SMOKE_READY_FIXTURE_PATH_PASSES_NOT_EVIDENCE`
Artifact ID: `PACKAGE_C_SIDEWALL_ROUTE_ACCEPTED_EVIDENCE_SMOKE_20260701`
Claim boundary: `fixture_smoke_not_evidence_not_route_score_not_yield_not_detection`

Fixture detector/wet rows: `2` / `14`.
Fixture path pass rows: `2`.
Component-vector ready rows: `2`.
route/yield/detection current rows: `0` / `0` / `0`.

This is a fixture-only smoke artifact. It proves the accepted-evidence code path can open the formula component vector, but the generated fixture rows are not evidence and are not written to the real input paths.
