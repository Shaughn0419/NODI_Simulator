...
计算侧的总口径也已经更新：Tsuyama 2022 selected-annulus Phase 2 / 2.11 不再继续无实测参数搜索。2026-05-14 复核修正了 Table S1 Ag/Au target：Ag interferometric 列本身就是 ratio-like 目标，不能再除以 Au 列；旧 strict over-ratio 只保留为 legacy audit。当前最清晰的结论是：corrected direct / formula-consistent Table S1 Ag/Au signal 已基本对齐，detection 不是主 blocker；真正未解的是 raw Au size-response 偏陡，残差主要集中在 Au40-Au60 slope。单一全局 response-compression lens 已接近 descriptive partial，但仍不能签 paper-calibrated raw candidate。因此后续工作不应继续扩大 D2/noise/threshold/paper-fit 搜索，而应转向 ET-2030 + LI5640 detector-unit feasibility、paper-matched finite-count/IQR 统计和真实 blank/BFP/Au trace 标定。
...
### 8.2 Table S1 Ag/Au ratio：修正后的 direct target 与 legacy over-ratio

2026-05-14 复核后确认，旧 `strict` 口径有计算语义错误：Tsuyama Table S1 的 Ag interferometric scattering 列本身已经是 Ag/Au ratio-like 数值，数值上等于 `sqrt(scattering_Ag/scattering_Au)`；不能再除以 Au interferometric 列。代码已修正为：`interferometric_column_ratio = Ag 表列本身`，旧的 `Ag 表列 / Au 表列` 只保留为 `legacy_interferometric_column_over_gold_ratio` 审计字段。

| wavelength | Table S1 Au inter | corrected Ag/Au target | sqrt-scattering cross-check | legacy over-ratio | 本轮 raw Ag/Au peak ratio | corrected gain |
|---|---:|---:|---:|---:|---:|---:|
| 488 nm | 0.45 | 1.90 | 1.897 | 4.222 | 1.460 | 1.301 |
| 532 nm | 0.68 | 0.89 | 0.897 | 1.309 | 0.686 | 1.298 |
| 660 nm | 0.32 | 0.85 | 0.837 | 2.656 | 0.853 | 0.996 |

因此，原先“旧 strict Table S1 需要 1.76-3.12x per-wavelength Ag transfer”的结论不再成立；那是在 double-normalized legacy over-ratio 上得到的诊断结果。Ag/Au mismatch 不是当前主要问题，更不能作为 Ag/Au 表面建模错误的证据。保守结论应改为：Table S1 Ag/Au signal 在 corrected direct / sqrt-scattering 口径下基本可解释；真正仍未解决的是 raw Au size-response 偏陡。
...
因此，Tsuyama 数值对齐目前最稳的是“660 nm Au selected-rate proxy 大体合理”和“Table S1 Ag/Au ratio 在 corrected direct / sqrt-scattering target 下 raw signal 已接近”。真正仍未解决的是 raw Au size-response：seed-median acceptance 下 best raw size score 仍约 `1.632`，没有 raw family 同时通过 signal 与 size。这个问题更可能需要 reference phase / BFP ROI / collection operator / pulse-readout 进一步诊断，而不应通过改 EV 全局默认来硬贴。
...
进一步按“只复核评分叙事、不新增仿真自由度”的边界做 Phase 2.8 reviewed/descriptive rescore：当时的 strict residual 实际对应 legacy over-ratio，现已不再作为 primary reproduction score；detection warning 与 fit complexity 也按描述性复现目标降权。最佳候选仍不变，reviewed score 降至 `1.9385`，状态为 `bounded_reproduction_partial_descriptive`。这个结果说明：如果目标只是描述“现有估计项能把 Tsuyama 数值贴近到什么程度”，当前低自由度 fit 已达到一个可读的 partial reproduction；但它不是 acceptance，不改变 `negative_or_diagnostic_result_only`，No-Go 仍是 `raw_size_response_alignment_not_met`，也不触发 `10000 events/case` confirm。

继续沿“路线 2”做 Phase 2.9 maximal upper-bound rescore：这一步历史上加入的是 hypothetical legacy-over-ratio Ag transfer，用来回答“若硬贴当时误读的 strict 表列，上限能到哪里”。3000-event size-only best 仍是 `tau_2ms_global_refphi_plus_0p6__paper_5sigma_size_response_fit`，maximal score `1.2869`，仅为 `maximal_paper_fit_partial_upper_bound`；full inverse best `refspace_0p25__paper_5sigma_sensitivity` score `0.9814`，D2.1 best `tau_2ms_global_refphi_plus_collection_narrow` score `0.9893`，二者可达 `maximal_paper_fit_upper_bound`。该结论现在只能作为“旧 double-normalized target 也能被高自由度项硬贴”的历史诊断，不再是推荐计算目标。release 仍是 `negative_or_diagnostic_result_only`，No-Go 仍是 `raw_size_response_alignment_not_met`。
