...
- 非授权: 本 closure 不授权 calibrated SNR/LOD/blank-FPR、detector-resolved winner、
  绝对跨波长 winner、生物真值；NODI 外差增强倍数不授权。
...
fast/vectorized path(C2)、route-specific readout/noise(M2/M3/#3)、以及 V2 raw-angular + 显式 ref/sca 归一化 tuple 生成(#4-#6)。
...
V2 raw-angular + 显式 ref/sca 归一化 tuple(#4–#6) 都仍需额外实现。
...
### 1.3 V1 gauge-locked vs V2 raw-angular + explicit norm(reviewer B 新 P1)
...
| **V2 raw-angular + explicit norm** | 用 raw `∫w\|s(θ)\|²`(s=`collection["angular_field_theta"]`, 已按投影; 不 rescale)，再显式记录 ref/sca 归一化 | **ref/sca 各自显式 norm mode** |
...
| gauge mode | V1_gauge_locked / V2_raw_angular_explicit_norm_sample | T1.3 |
...
`collapsed_ref_raw`/`collapsed_sca_raw`, 故 raw-angular + 显式归一化 C **可定义**, 只是 cost 上可省略 ——
...
candidate-family 边界；A/B `V2_raw_angular_explicit_norm_sample` 仅作为抽样 gauge 检验。任何 calibrated heterodyne /
...
**判决**: 若 family 随 gauge_ratio 翻转 → 不得把任何 "NODI 外差增强倍数" 写成已证明 claim；若未来只作历史措辞引用，也必须带 gauge 注脚(已在 147 §9)。
...
- `report148_stage1_preseal_manifest.json`: `sealing_gate_status =
  met_under_narrowed_no_data_scope`，`narrowed_no_data_gate_met = true`，
  `full_route_readout_gauge_closure_met = false`，`out_of_scope_cells = 10`。
