...
- **下一轮 A/B/C 必须分两版(见 §7): V1 gauge-locked(保留 rescale, 测工程内部 detector surrogate
  的 ranking stability) 与 V2 raw-angular + 显式 ref/sca 归一化(不 rescale, 测归一化口径下的 route/gauge 敏感性)。
...
**gauge / scale 差异本身不会自动成为 detector-identity headline。** 当前实现里 V1↔V2 raw-angular + 显式归一化
...
- **V2 raw-angular + explicit norm**: 不 rescale, 直接用 raw angular ref/sca 场并显式记录 ref/sca 归一化 → 测归一化口径下的 route/gauge 敏感性。
  **V2 必须显式定义 raw angular 振幅如何归一化**(否则 V2 的 ranking flip 可能只是 normalization flip,
  不是 detector identity flip); 单字段 `angular_field_normalization_mode` 已视为 deprecated，
  当前口径应拆成 `raw_reference_normalization_mode`、`raw_scattering_normalization_mode`、
  `route_normalization_anchor`, `route_level_normalization_view`。
...
**判决规则:**
- **V2 只在同一 `(raw_reference_normalization_mode, raw_scattering_normalization_mode)` 组合内比 ranking; 跨 normalization 的差异是 diagnostic, 不是 final ranking 证据。**
...
A/B/C/D 全 3-seed；A/B 的 `R2/V2_raw_angular_explicit_norm_sample` 也为 3/3 seed 完整，作为抽样 gauge 检验。consensus 复现为
...
manifest 明确登记 `narrowed_no_data_gate_met = true` 且
`full_route_readout_gauge_closure_met = false`。
