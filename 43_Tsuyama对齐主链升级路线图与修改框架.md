...
parameter_sweep.py:
  ...
  design_postprocess.py 接出去，不要把所有设计语义塞回单 case score。
...
| 方向 | 当前代码事实 | 结论 |
|---|---|---|
| EV ensemble / sample uncertainty | `structured_particles.py` 已有 biomimetic EV preset 和 explicit ensemble builder；`utils.py` 已输出 EV claim、shape/corona/isolation uncertainty blocker。 | 需要从 metadata 升级为 design aggregation 与 sample-prep-weighted ensemble。 |
| EV reporting / preanalytical metadata | `ev_reporting_metadata.py` 已接入，能输出 EV reporting readiness / sample-prep provenance / biological-claim blocker。 | 当前可区分 EV-like optical particle 与 biological EV claim；无真实 metadata 时仍阻塞 biological specificity。 |
...
| Standard particle ladder | `utils.py` 已有 standard-particle calibration table、synthetic guard、uncertainty blockers 和 held-out validation blocker；但没有 PS/silica/liposome ladder 与 fit/validation/challenge split。 | P0 增加 ladder schema；P2 接实测 standard traces。 |
...
| Particle-channel double counting | `particle_channel_perturbation.py` 已输出 double-counting risk 与 guard 字段。 | 默认不把 perturbation 加回主 score；只有 guard 通过或高保真/实测闭环后才可升级。 |
| Detector forward model | `detector_forward_model` 已接受 `collapsed_scalar_surrogate`、`joint_overlap_coherent_surrogate`、`roi_intensity_integral`、`roi_complex_mode_overlap_integral`；`bfp_detector_operator.py` 已输出 scalar-vs-ROI disagreement。 | ROI lane 是 comparison/diagnostic contract；无 detector-unit chain 时仍不能给 photon/voltage calibrated claim。 |
| Tsuyama BFP reference | `tsuyama_phase_filter.py` 已输出 BFP 复场；`reference_field.py` 已提供 `tsuyama_bfp_integrated` detector-resolved comparison lane。 | 可做 BFP-level comparison，不替换 measured blank calibration 或 calibrated truth。 |
| Reference operating point | `reference_operating_point.py` 已输出 reference too weak / saturation / RIN/leakage risk 等工作带诊断。 | 防止“reference 越强越好”的错误推荐；无实测 detector/noise budget 时仍是诊断。 |
| NA cutoff | `reference_field.py` 当前有 engineering hard guardrail/width saturation。 | 增加 soft rolloff policy，保留 hard guardrail 作保守 lane。 |
...
| Fluidics / clogging | `fluidic_resistance.py`、`fluidic_network_model.py` 与 design metrics 已输出 hydraulic / practicality diagnostic。 | 当前是静态 practicality / resistance 诊断；fouling time evolution 仍属 P2。 |
| Channel real geometry | `channel_geometry_model.py` 已输出 geometry model schema、rounded/trapezoid/measured profile readiness 与 discrepancy flag。 | 真实 profile lookup / full geometry solver 仍需 P1/P2 数据或模型。 |
| Electrokinetic / surface charge | `electrokinetic_transport.py` 已输出 Debye/wall-potential diagnostic 与 claim blocker。 | transport 仍未用完整 Poisson-Boltzmann/wall-exclusion 主求解，保持 blocker。 |
| EV integrity / deformation | `ev_integrity_risk.py` 与 exposure/fluidic diagnostics 已输出 EV integrity risk / gate。 | 可阻止破坏性设计进绿色；非实测 integrity 仍是风险诊断。 |
| Interface correction | `interface_correction.py` 已有 boundary、dipole/fullwave blocker。 | P1 实现 dipole image surrogate；P3 再做 dyadic Green/fullwave lookup。 |
| Readout / threshold / event QC | `readout_transfer_model.py`、threshold/blank boundary、raw blank bootstrap boundary 与 `event_quality_control.py` 已接入；matched filter 仍非主线。 | 当前已能输出 event artifact risk / readout semantics blocker；P2 接实测 blank trace。 |
| Detector units / noise | `detector_units.py` 是 boundary；RIN/speckle/leakage/stray options 多为 not_applied。 | P1 做 relative noise components；P2 detector voltage/photon chain 解锁。 |
| POD / NODI thermal contamination | `photothermal_pod.py` 仍是 quantitative boundary；`nodi_thermal_contamination.py` 已输出 NODI thermal contamination proxy。 | P2/P3 做 thermal POD solver/lookup；当前不解锁热定量 amplitude。 |
| Materials / media / wavelength lanes | `materials.py` 已含 Au/Ag、water/PBS/HEPES/culture/sucrose/iodixanol、fused silica/Viosil/BK7 等 nominal properties；`wavelength_comparability.py` 已接入。 | 可输出 wavelength-lane claim blocker；无实测跨波长 detector-unit chain 时不能做真实跨波长优劣 claim。 |
| Contaminants / classification | `particle_design_library.py` 已包含 contaminant / standard-particle presets 与 design library diagnostics。 | EV_vs_contaminant AUC / unknown rejection 仍是 bounded diagnostic，不是实测分类器。 |
| Selection function / observed distribution | `selection_function.py` 已输出 skeleton 和 observed-distribution correction diagnostic。 | 可暴露 selection bias blocker；P3 population inference 仍需数据。 |
| OOD / unknown events | `ood_detection.py` 已接入 unknown/reject scaffold。 | 当前是 OOD diagnostic，不是训练好的实测分类器。 |
| Uncertainty / sensitivity | `uncertainty.py` 仍是 propagation boundary；`design_postprocess.py` 已输出 model consensus / route consensus 等设计层诊断。 | Sobol/Morris 与实测 posterior 仍属 P1/P2。 |
| Count-rate semantics | `count_generation.py` 已隔离 per-event detectability；`count_likelihood.py` 已提供 observed-count likelihood scaffold。 | count-rate confidence 仍需 concentration/flow/blank/dead-time 实测闭环。 |
| Population full-trace | `population_trace_simulator.py` 已提供 population full-trace skeleton/smoke。 | 防止 isolated detectability 被误读为真实 run；P3 才做正式 population inference。 |
| Run state / drift | `run_state_model.py` 已输出 stationarity/fouling/reblank interval scaffold。 | 实测 blank/run trace 校准仍属 P2。 |
| Unit / axis conventions | `unit_conventions.py` 已输出 unit/axis convention hard gate 与 Mie validation diagnostics。 | 当前用于 schema/gate；继续保持测试锁定。 |
| Objective / optical hardware | `optical_hardware_profiles.py` 与 `objective_panel.py` 已接入 objective profile / panel diagnostics。 | 可做 objective comparison scaffold；真实硬件 throughput 仍需校准。 |
| Optical exposure safety | `optical_exposure_safety.py` 已输出 exposure safety gate / safe-power claim level。 | 无热/photodamage 实测时仍是 conservative diagnostic。 |
| NODI readout semantics | 当前 readout 有 sampled carrier、analytic lockin surrogate、measured transfer function；`utils.py` 已标记 NODI source 是 transient scattering surrogate，不是 carrier-modulated source。 | P0 增加 `readout_transfer_model.py` 与 nonsynchronized NODI semantics blocker；EV design 默认不得使用 locked-carrier surrogate 解锁 claim。 |
| Polarization / Jones | `polarization_jones_operator.py` 已接入 scalar/Jones/measured-matrix scaffold。 | measured Jones matrix 才解锁 phase/polarization quantitative claim。 |
| Calibration planning | `bayesian_calibration.py`、`calibration_plan_advisor.py`、`experimental_design_advisor.py` 已接入 posterior / VOI scaffold。 | P2 接实测 posterior。 |
| Assay controls | `assay_control_matrix.py` 与 `control_interpretation.py` 已输出 buffer/medium/EV-depleted/lysis/spike-in/dilution control scaffold。 | 推荐输出可带 assay-control blockers；真实控制样本结果仍待接入。 |
| Recompute reproducibility | `recompute_manifest.py` 与 dashboard/precompute metadata/checkpoint 已输出 manifest/hash/schema inventory 与 performance context。 | 正式 full-grid 仍需实际重算生成 current library。 |
...
新增 `particle_channel_perturbation.py`。
...
在 `reference_field.py` 或 `detector_noise_budget.py` 中增加：
...
新增 `design_metrics.py` 或放入 `design_postprocess.py` 的 row metric 层：
...
新增 `wavelength_comparability.py`，把“跨波长/跨物镜能不能比较”从 dashboard 文案升级为硬 blocker。现有 `normalization_mode="per_wavelength"` 适合在同一 lambda 下比较 W/H，但不能证明 404/488/532/660 nm 的真实 detector voltage、真实 SNR 或真实 LOD 谁更优。

扩展 `materials.py` 与 `Medium` / `SimulationConfig`：
...
新增 `assay_control_matrix.py` skeleton。首版只输出控制样品清单和 claim blocker，不需要实测 trace：
...
新增 `selection_function.py`。P0 只建立 schema 和基础计算，不做完整 population inversion。
...
新增 `channel_geometry_model.py` skeleton：
...
新增 `electrokinetic_transport.py` skeleton：
...
新增 `ev_integrity_risk.py` skeleton：
...
新增 `recompute_manifest.py` 或扩展 dashboard/precompute manifest。
...
新增 `readout_transfer_model.py`，把 NODI transient pulse 与 POD locked modulation 分清楚。Tsuyama-style simultaneous POD/NODI 读出可以用 lock-in 电子学描述，但 EV/NODI pulse 本身不是稳定相位锁定载波；它更接近 transient pulse 经过 bandpass/lock-in transfer function 后的 envelope/IQ 响应。
...
新增 `unit_conventions.py` 和 `tests/test_unit_conventions.py`，把 radius/diameter、W/H、坐标轴和 flow/optical axis 约定做成 hard gate。
...
新增 `optical_hardware_profiles.py` 和 `objective_panel.py`。现有 `OpticalSystem` 参数继续保留，但全量设计输出必须知道这些参数来自哪个 objective/hardware profile，而不是隐含固定为 20x NA0.45。
...
新增 `optical_exposure_safety.py`。这条 gate 独立于 POD thermal solver 和 NODI thermal contamination：即使 EV 主路是 NODI-only，短波长、高功率、小 waist 也可能带来 heating、EV membrane damage、thermal lens、bubble 或 Au standard thermal artifact。
...
8. Counting objective / coincidence refinement
   - 在现有 `count_generation.py` 基础上新增 `single_event_detectability_score` 与 `experiment_counting_score`
...
11. `calibration_plan_advisor.py` static blocker-to-experiment map
...
12. Selection-function-based observed distribution correction
   - 用 P0 `selection_function.py` 输出 true-vs-observed bias
...
17. `nodi_thermal_contamination.py`
...
18. `polarization_jones_operator.py` interface
...
25. Random-arrival-phase NODI readout simulation
   - 在 `readout_transfer_model.py` 上增加 `random_arrival_phase_lockin`
...
静态“缺什么该测什么”的清单生成属于 P1 `calibration_plan_advisor.py`；P2 只负责将真实实验结果接回 calibration state。
...
```text
particle_channel_perturbation.py
bfp_detector_operator.py
detector_noise_budget.py
fluidic_resistance.py
design_metrics.py
design_postprocess.py
alignment_fabrication_robustness.py
sensitivity_analysis.py
classification_objectives.py
inverse_problem_diagnostics.py
ev_ri_inference.py
calibration_plan_advisor.py
thermal_pod_solver.py
wavelength_comparability.py
readout_transfer_model.py
unit_conventions.py
optical_hardware_profiles.py
objective_panel.py
optical_exposure_safety.py
population_trace_simulator.py
ev_population_prior.py
fluidic_network_model.py
control_interpretation.py
ev_reporting_metadata.py
selection_function.py
standard_particle_ladder.py
event_quality_control.py
count_likelihood.py
channel_geometry_model.py
electrokinetic_transport.py
ev_integrity_risk.py
run_state_model.py
nodi_thermal_contamination.py
polarization_jones_operator.py
bayesian_calibration.py
experimental_design_advisor.py
ood_detection.py
population_inference.py
assay_control_matrix.py
recompute_manifest.py
```
