"""
NODI Interferometric Simulator — Parameter Sweep Module
...
def _optional_numba_njit(*args, **kwargs):
...
def _wilson_half_width(successes: int, total: int) -> float:
    """Return half-width of the Wilson interval for a binomial estimate."""
    if total <= 0:
        return 1.0
    return 0.5 * (
        wilson_upper_bound(successes, total)
...
def _mark_numpy_arrays_readonly(payload: dict) -> dict:
    """Prevent accidental mutation of cached numerical arrays."""
...
def _get_or_compute_reference_field(
    ...
    if cached is None:
        ...
        reference_cache[key] = deepcopy(reference)
        return reference
